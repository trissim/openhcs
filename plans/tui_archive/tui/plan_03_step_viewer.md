# plan_03_step_viewer.md
## Component: Step Viewer Pane

### Objective
Implement the Step Viewer pane that displays the flat list of all FuncStep declarations for the selected plate, with visual separators between pipelines, supporting keyboard navigation and click selection.

### Plan

1. **Step List Component**
   - Implement a scrollable, selectable list of FuncStep declarations
   - Display step names, types, and basic metadata
   - Support both mouse and keyboard navigation
   - Include visual separators between pipelines

2. **Step Selection Mechanism**
   - Maintain selection state in the TUI state manager
   - Emit selection events when a step is highlighted
   - Provide visual feedback for the currently selected step

3. **Step Detail View**
   - Display detailed information for the selected step
   - Show function patterns, parameters, and memory types
   - Include edit button to launch Function Pattern Editor

4. **Pipeline Visualization**
   - Implement visual separators between pipelines
   - Display pipeline metadata (name, ID, status)
   - Use consistent styling for pipeline grouping

5. **Step Status Indicators**
   - Display status icons for each step (pending, validated, error)
   - Update status based on compilation results
   - Provide tooltips for status explanation

6. **Step Reordering**
   - Implement controls to move steps up and down within a pipeline
   - Provide visual feedback during reordering
   - Maintain pipeline boundaries during reordering
   - Update pipeline execution order after reordering

### Findings

#### Key Considerations for Step Viewer

1. **FuncStep Representation**
   - FuncStep objects contain function patterns that can be complex
   - The viewer should display a simplified representation
   - Detailed editing should happen in the Function Pattern Editor
   - Example: `step_name: function_name (backend) → output_memory_type`

2. **Pipeline Separation**
   - Multiple pipelines may be defined for a plate
   - Visual separators should clearly distinguish between pipelines
   - Example: Use horizontal lines with pipeline names as labels

3. **Step Selection**
   - Selection should trigger updates in other panes
   - Selected step should be highlighted
   - Example: Highlight the selected step and scroll to ensure visibility

4. **Memory Type Visualization**
   - Memory types are critical in OpenHCS
   - The viewer should clearly indicate input and output memory types
   - Example: Use color coding or icons to indicate memory types

5. **Edit Integration**
   - Editing a step should launch the Function Pattern Editor
   - The editor should replace the Plate Manager pane temporarily
   - Example: Click "Edit" button to switch to Function Pattern Editor
...
from ezstitcher.core.steps.function_step import FunctionStep

...
### Objective
Implement a separate dialog manager component for handling plate-related dialogs, using composition to maintain clear boundaries with the Plate Manager pane. This component will focus exclusively on dialog creation, display, and lifecycle management.

### Plan

1. **Dialog Manager Class**
   - Create a separate PlateDialogManager class
   - Use composition with callback interfaces
   - Maintain clear boundaries with the Plate Manager pane
   - Provide a clean API for dialog operations

2. **Dialog Creation**
   - Implement file browser dialog with backend selection
   - Create confirmation dialogs for plate removal
   - Implement error dialogs with detailed information
   - Support customizable dialog options

3. **Dialog Lifecycle Management**
   - Implement proper dialog display and closure
   - Ensure focus is restored after dialog closure
   - Handle dialog cancellation properly
   - Provide consistent dialog behavior

4. **Callback Interface**
   - Define clear callback protocols for communication
   - Implement event-based communication with parent component
   - Maintain separation of concerns
   - Avoid direct dependencies on parent component

### Findings

#### Key Considerations for Dialog Manager
...
2. **🔒 Backend Selection (Clause 316)**
   - UI must force explicit backend selection
   - No implicit defaults for backend selection
   - Backend selection must map directly to read_backend/write_backend
   - Implementation: `backend_selector = Dropdown(options=[("disk", "Local Disk"), ...])`

3. **🔒 Dialog Lifecycle Management**
   - Ensure proper dialog creation and disposal
   - Restore focus after dialog closure
   - Handle dialog cancellation properly
   - Prevent dialog leaks and orphaned dialogs

4. **🔒 Error Handling**
   - Provide detailed error information
   - Show error dialogs with option to view logs
   - Ensure exceptions are properly propagated
   - Use callbacks for error notification
...
from prompt_toolkit.layout import HSplit, Float, Container, ConditionalContainer
from prompt_toolkit.widgets import Button, Label, TextArea, Dialog, RadioList as Dropdown, Box

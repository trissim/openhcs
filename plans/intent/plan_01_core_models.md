# plan_01_core_models.md
## Component: Intent Core Models

### Objective
Create the core models for representing and organizing intents extracted from code structure. These models will serve as the foundation for the entire Structural Intent Analysis system.

### Plan
1. Create a base `Intent` class that represents extracted intent from any source
2. Create specialized intent classes for different sources (names, types, structure)
3. Implement an `IntentHierarchy` class for organizing intents in a hierarchical structure
4. Implement serialization/deserialization for all intent models
5. Create utility functions for working with intents

### Findings
The intent models need to be flexible enough to represent various types of intent extracted from different sources, while also providing a consistent interface for working with intents throughout the system.

Key requirements:
- Intents should have a unique identifier
- Intents should have a source (where they were extracted from)
- Intents should have a confidence score
- Intents should be able to reference related intents
- Intents should be serializable/deserializable

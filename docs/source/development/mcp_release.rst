...
* hosted domain, deployment credentials, and public privacy/legal/support URLs.

Private hosted deployments additionally require an OAuth issuer and
introspection credentials. The initial universal public plugin does not.

Hosted Streamable HTTP staging
------------------------------

The hosted entry point is ``openhcs-mcp-http`` from the ``mcp-http`` extra.
It is a separate, registry-filtered Streamable HTTP service, not the local
stdio server exposed through a public port.

The initial ChatGPT web plugin uses ``public_read_only``. Configure the
production service with:

.. code-block:: text

   OPENHCS_MCP_HTTP_PUBLIC_URL=https://mcp.example.org/mcp
   OPENHCS_MCP_HTTP_AUTH_MODE=public_read_only
   OPENHCS_MCP_HTTP_ALLOWED_HOSTS=mcp.example.org
   OPENHCS_MCP_HTTP_ALLOWED_ORIGINS=https://chatgpt.com
   OPENHCS_MCP_HTTP_RESOURCE_DOCUMENTATION_URL=https://openhcs.readthedocs.io/
   OPENHCS_AGENT_READ_ROOTS=/srv/openhcs/package:/srv/openhcs/data
   OPENHCS_AGENT_WRITE_ROOTS=/srv/openhcs/data

Server construction rejects either hosted mode if the capability registry ever
contains a mutating or side-effecting tool. Authentication changes who may read
the surface, not its mutation boundary. The access policy is projected into
each tool's current and compatibility security-scheme metadata without copying
tool names.

``packaging/hosted-mcp/Dockerfile`` installs an exact released PyPI version,
runs as an unprivileged user, and defaults to this mode. Build it only after
that exact OpenHCS release is public:
...
.. code-block:: text
   ...
   OPENHCS_MCP_HTTP_PUBLIC_URL=https://mcp.example.org/mcp
   OPENHCS_MCP_HTTP_AUTH_MODE=oauth_introspection
...
``openhcs.mcp.audit`` logger with the authentication mode, nominal capability
identity, transport, outcome, and timestamp. Private OAuth events additionally
carry the configured tenant subject. Route that logger to a durable restricted
sink; never add bearer tokens or tool arguments to it.
...
mode. Production deployment still requires the domain, certificate, secrets,
audit retention, monitoring, and plugin registration. Private mode additionally
requires the external authorization server and per-tenant workspace isolation.

The service always exposes an unauthenticated ``/healthz`` route containing
only status, transport, and authentication mode. When the submission portal
provides a domain-verification token, set:

.. code-block:: text

   OPENHCS_MCP_HTTP_OPENAI_DOMAIN_CHALLENGE_TOKEN=<exact-portal-token>

The server then returns that exact token at
``/.well-known/openai-apps-challenge``. Do not invent the token or return a JSON
wrapper around it.

Browser plugin artifact
-----------------------

After the public domain is live, register its ``/mcp`` endpoint in ChatGPT
developer mode and copy the resulting ``plugin_asdk_app_...`` identity. Generate
the remote-only package and deterministic reviewer packet from synchronized
release metadata:
...
.. code-block:: bash
   ...
   python scripts/build_hosted_mcp_plugin.py \
     ...
     --authentication-mode public_read_only \
     --website-url https://openhcs.example/ \
     --support-url https://openhcs.example/support.html \
     --privacy-policy-url https://openhcs.example/privacy.html \
     --terms-of-service-url https://openhcs.example/terms.html \
     --output-dir dist/chatgpt/openhcs

The generated package maps the registered remote app through ``.app.json`` and
contains read-only product metadata, one checked brand asset, and the review
packet. It does not bundle the local workflow skill, advertise write/UI
capabilities, or copy a tool-name allowlist. Public submission still uses the
OpenAI Platform **With MCP** flow and scans the production endpoint directly;
``submission.json`` is a review aid, not a runtime authority.

Before submission, verify the endpoint with MCP Inspector and ChatGPT developer
mode, run every generated positive and negative review case, confirm tool
annotations and security schemes in the portal scan, and verify the public
website, support, privacy, and terms URLs. Public review also requires a
verified OpenAI developer or business identity and Apps Management write
permission.

Recheck the current OpenAI requirements before each submission:

* `Build an MCP server
  <https://developers.openai.com/plugins/build/mcp-server>`_;
* `Connect from ChatGPT
  <https://developers.openai.com/plugins/deploy/connect-chatgpt>`_;
* `Submit a plugin
  <https://developers.openai.com/plugins/deploy/submission>`_; and
* `App review requirements
  <https://developers.openai.com/plugins/deploy/app-review>`_.

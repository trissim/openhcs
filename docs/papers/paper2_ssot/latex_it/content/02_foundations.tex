...
This section introduces only the minimum structure needed to state the deterministic converse foundation and its threshold corollary: latent states, deterministic observations, auxiliary tags, ambiguity classes, and independent rate.
...
Let $E(C)$ denote the admissible edit set of encoding system $C$. Two locations encoding the same fact are \emph{independent} if there exists a sequence of edits from $E(C)$ that makes them disagree.
\end{definition}
...
The \emph{degrees of freedom} (DOF), or \emph{independent rate}, of an encoding system for fact $F$ is the number of independently writable locations encoding $F$ under the admissible edit set. In the single-fact model, the remaining encoding locations are treated as derived views of those independent sources.
\end{definition}
...
\end{corollary}
\leanmeta{\LH{RAT1}, \LHrng{CAP}{2}{3}}
...
This threshold is an early architectural consequence of the deterministic model. The deeper theorems of the paper begin once one asks how much finite side information is needed whenever the transcript leaves a nontrivial ambiguity class.
...
The remainder of the paper sharpens this counting statement into a unified finite converse family, extends it to non-clique confusability graphs, and then develops the corresponding asymptotic capacity and upper-bound theory.

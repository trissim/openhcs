...
\begin{proof}
Pair injectivity is the structural core. The confusability and counting theorems express the injectivity requirement combinatorially; the conditional-entropy theorem expresses the same obstruction as residual uncertainty; the decoder-output and gap theorems express it through deterministic coarsening and finite alphabet ceilings. Each theorem therefore measures the same failure of exact isolation in a different coordinate system.
\end{proof}
...
The zero-error theorems are the $P_e=0$ boundary of this inequality. In that regime the success branch occupies all probability mass, the failure term disappears, and the budget collapses back to the finite converse family above.

...
theorem sideInfoBits_strict_mono {m n : Nat} (hm : m > 0) (hmn : m < n) :
  ...
  exact div_lt_div_of_pos_right hlog hlog2_pos
...
theorem sideInfoBits_eq_zero_iff {k : Nat} (hk : k > 0) :
  ...
  · intro h
    ...
    have hlogk : Real.log (k : ℝ) = 0 := by
      have h' := congrArg (fun x => x * Real.log 2) h
      simpa [mul_div, hlog2_ne] using h'
    have hkpos : (0 : ℝ) < (k : ℝ) := by exact_mod_cast hk
    exact_mod_cast (Real.eq_one_of_pos_of_log_eq_zero hkpos hlogk)
  · intro hk
    subst hk
    unfold sideInfoBits

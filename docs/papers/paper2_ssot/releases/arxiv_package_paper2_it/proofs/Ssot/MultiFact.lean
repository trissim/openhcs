...
/-- The upward-closed family of agreement sets generated by a view family. -/
def agreementUpwardFamily {F L : Nat} (views : ViewFamily F L) : Set (Finset (Fin F)) :=
  fun S => ∃ ℓ : Fin L, views ℓ ⊆ S

/-- Upward closure on coordinate-subset families. -/
def IsUpwardClosed {F : Nat} (U : Set (Finset (Fin F))) : Prop :=
  ∀ ⦃S T : Finset (Fin F)⦄, S ∈ U → S ⊆ T → T ∈ U

theorem agreementUpwardFamily_upwardClosed
    {F L : Nat} (views : ViewFamily F L) :
    IsUpwardClosed (agreementUpwardFamily views) := by
  intro S T hS hST
  rcases hS with ⟨ℓ, hℓ⟩
  exact ⟨ℓ, Finset.Subset.trans hℓ hST⟩

theorem confusable_iff_agreeSet_mem_agreementUpwardFamily
    {F A L : Nat} (views : ViewFamily F L) {x y : State F A} :
    Confusable views x y ↔ x ≠ y ∧ agreeSet x y ∈ agreementUpwardFamily views := by
  simpa [agreementUpwardFamily] using
    (confusable_iff_exists_view_subset_agreeSet (A := A) views (x := x) (y := y))

noncomputable def viewFamilyOfAgreementUpwardFamily {F : Nat}
    (U : Set (Finset (Fin F))) :
    ViewFamily F (Fintype.card (Finset (Fin F))) := by
  classical
  intro i
  let S : Finset (Fin F) := (Fintype.equivFin (Finset (Fin F))).symm i
  exact if S ∈ U then S else Finset.univ

theorem exists_view_eq_viewFamilyOfAgreementUpwardFamily
    {F : Nat} (U : Set (Finset (Fin F))) {S : Finset (Fin F)}
    (hS : S ∈ U) :
    ∃ ℓ : Fin (Fintype.card (Finset (Fin F))),
      viewFamilyOfAgreementUpwardFamily U ℓ = S := by
  classical
  let e := Fintype.equivFin (Finset (Fin F))
  refine ⟨e S, ?_⟩
  unfold viewFamilyOfAgreementUpwardFamily
  simp [e, hS]

theorem confusable_viewFamilyOfAgreementUpwardFamily_iff
    {F A : Nat} (U : Set (Finset (Fin F)))
    (hU : IsUpwardClosed U) {x y : State F A} :
    Confusable (A := A) (viewFamilyOfAgreementUpwardFamily U) x y ↔
      x ≠ y ∧ agreeSet x y ∈ U := by
  classical
  constructor
  · intro hxy
    rcases (confusable_iff_exists_view_subset_agreeSet
      (A := A) (viewFamilyOfAgreementUpwardFamily U)).mp hxy with ⟨hneq, ℓ, hsub⟩
    let S : Finset (Fin F) := (Fintype.equivFin (Finset (Fin F))).symm ℓ
    have hS_cases : S ∈ U ∨ S ∉ U := by exact em (S ∈ U)
    rcases hS_cases with hSU | hSnotU
    · have hview : viewFamilyOfAgreementUpwardFamily U ℓ = S := by
        unfold viewFamilyOfAgreementUpwardFamily
        simp [S, hSU]
      exact ⟨hneq, hU hSU (by simpa [hview] using hsub)⟩
    · have hview : viewFamilyOfAgreementUpwardFamily U ℓ = Finset.univ := by
        unfold viewFamilyOfAgreementUpwardFamily
        simp [S, hSnotU]
      have huniv : agreeSet x y = (Finset.univ : Finset (Fin F)) := by
        apply Finset.Subset.antisymm
        · exact Finset.subset_univ _
        · simpa [hview] using hsub
      have hxy : x = y := by
        funext i
        have hi : i ∈ agreeSet x y := by simpa [huniv]
        exact mem_agreeSet_iff.mp hi
      exact False.elim (hneq hxy)
  · rintro ⟨hneq, hmem⟩
    rcases exists_view_eq_viewFamilyOfAgreementUpwardFamily U hmem with ⟨ℓ, hℓ⟩
    refine (confusable_iff_exists_view_subset_agreeSet
      (A := A) (viewFamilyOfAgreementUpwardFamily U)).mpr ?_
    refine ⟨hneq, ℓ, ?_⟩
    simpa [hℓ]

/-- A graph relation on the full tuple space is classified by an upward-closed family of
agreement sets when adjacency is exactly determined by agreement-set membership in that family. -/
def AgreementClassified {F A : Nat} (adj : State F A → State F A → Prop) : Prop :=
  ∃ U : Set (Finset (Fin F)),
    IsUpwardClosed U ∧ ∀ x y : State F A, adj x y ↔ x ≠ y ∧ agreeSet x y ∈ U

theorem confusable_agreementClassified
    {F A L : Nat} (views : ViewFamily F L) :
    AgreementClassified (F := F) (A := A) (Confusable views) := by
  refine ⟨agreementUpwardFamily views, agreementUpwardFamily_upwardClosed views, ?_⟩
  intro x y
  exact confusable_iff_agreeSet_mem_agreementUpwardFamily (A := A) views

theorem exists_viewFamily_iff_agreementClassified
    {F A : Nat} (adj : State F A → State F A → Prop) :
    (∃ L : Nat, ∃ views : ViewFamily F L,
        ∀ x y : State F A, adj x y ↔ Confusable (A := A) views x y) ↔
      AgreementClassified (F := F) (A := A) adj := by
  constructor
  · rintro ⟨L, views, hviews⟩
    refine ⟨agreementUpwardFamily views, agreementUpwardFamily_upwardClosed views, ?_⟩
    intro x y
    rw [hviews x y]
    exact confusable_iff_agreeSet_mem_agreementUpwardFamily (A := A) views
  · rintro ⟨U, hU, hUchar⟩
    refine ⟨Fintype.card (Finset (Fin F)), viewFamilyOfAgreementUpwardFamily U, ?_⟩
    intro x y
    rw [hUchar x y]
    exact (confusable_viewFamilyOfAgreementUpwardFamily_iff (A := A) U hU).symm

theorem agreeSet_eq_univ_iff
    {F A : Nat} {x y : State F A} :
    agreeSet x y = (Finset.univ : Finset (Fin F)) ↔ x = y := by
  constructor
  · intro hxy
    funext i
    have hi : i ∈ agreeSet x y := by simpa [hxy]
    exact mem_agreeSet_iff.mp hi
  · intro hxy
    subst hxy
    ext i
...
theorem agreeSet_ne_univ_of_ne
    {F A : Nat} {x y : State F A} (hxy : x ≠ y) :
    agreeSet x y ≠ (Finset.univ : Finset (Fin F)) := by
  intro hEq
  exact hxy ((agreeSet_eq_univ_iff.mp hEq))

theorem exists_distinct_states_with_agreeSet_eq_iff
    {F A : Nat} (hA : 1 < A) {S : Finset (Fin F)} :
    (∃ x y : State F A, x ≠ y ∧ agreeSet x y = S) ↔
      S ≠ (Finset.univ : Finset (Fin F)) := by
  constructor
  · rintro ⟨x, y, hxy, hS⟩
    intro hEq
    apply agreeSet_ne_univ_of_ne (F := F) (A := A) hxy
    simpa [hS] using hEq
  · intro hS
    have hA0 : 0 < A := lt_trans (by decide : 0 < 1) hA
    let zeroA : Fin A := ⟨0, hA0⟩
    let oneA : Fin A := ⟨1, hA⟩
    have hzero_one : zeroA ≠ oneA := by
      intro hEq
      have : (0 : Nat) = 1 := Fin.mk.inj hEq
      cases this
    have hnotall : ¬ ∀ i : Fin F, i ∈ S := by
      intro hall
      apply hS
      ext i
      simp [hall i]
    ...
    let y : State F A := fun j => if j ∈ S then zeroA else oneA
    refine ⟨x, y, ?_, ?_⟩
    · intro hxy
      have hcoord := congrArg (fun z => z i) hxy
      simp [x, y, hi, hzero_one] at hcoord
    · ext j
      by_cases hj : j ∈ S
      · simp [agreeSet, x, y, hj]
      · simp [agreeSet, x, y, hj, hzero_one]

theorem upwardClosed_family_eq_on_proper_subsets_of_same_relation
    {F A : Nat} (hA : 1 < A)
    {U V : Set (Finset (Fin F))}
    (hrel : ∀ x y : State F A,
      (x ≠ y ∧ agreeSet x y ∈ U) ↔ (x ≠ y ∧ agreeSet x y ∈ V))
    {S : Finset (Fin F)} (hS : S ≠ (Finset.univ : Finset (Fin F))) :
    S ∈ U ↔ S ∈ V := by
  rcases (exists_distinct_states_with_agreeSet_eq_iff (F := F) (A := A) hA).mpr hS with
    ⟨x, y, hxy, hagree⟩
  have hxyrel := hrel x y
  simpa [hxy, hagree] using hxyrel
...
def agreementOracle {F A L : Nat}
    (views : ViewFamily F L) (x y : State F A) : Bool :=
  decide (x ≠ y ∧ ∃ ℓ : Fin L, views ℓ ⊆ agreeSet x y)

def agreementOracleViewWork {F L : Nat} (views : ViewFamily F L) : Nat :=
  ∑ ℓ : Fin L, (views ℓ).card

def agreementOracleTotalWork {F L : Nat} (views : ViewFamily F L) : Nat :=
  F + agreementOracleViewWork views

theorem agreementOracle_eq_true_iff
    {F A L : Nat} (views : ViewFamily F L) {x y : State F A} :
    agreementOracle (A := A) views x y = true ↔ Confusable (A := A) views x y := by
  simp [agreementOracle, confusable_iff_exists_view_subset_agreeSet]

theorem agreementOracle_eq_confusableOracle
    {F A L : Nat} (views : ViewFamily F L) (x y : State F A) :
    agreementOracle (A := A) views x y = confusableOracle (A := A) views x y := by
  by_cases h : Confusable (A := A) views x y
  · have h1 : agreementOracle (A := A) views x y = true :=
      (agreementOracle_eq_true_iff views).2 h
    have h2 : confusableOracle (A := A) views x y = true := by
      simp [confusableOracle, h]
    simp [h1, h2]
  · have h1 : agreementOracle (A := A) views x y = false := by
      cases hbool : agreementOracle (A := A) views x y <;> simp at *
      exact False.elim (h ((agreementOracle_eq_true_iff views).1 hbool))
    have h2 : confusableOracle (A := A) views x y = false := by
      simp [confusableOracle, h]
    simp [h1, h2]

theorem agreementOracleViewWork_le
    {F L : Nat} (views : ViewFamily F L) :
    agreementOracleViewWork views ≤ L * F := by
  classical
  unfold agreementOracleViewWork
  calc
    ∑ ℓ : Fin L, (views ℓ).card ≤ ∑ _ℓ : Fin L, F := by
      exact Finset.sum_le_sum (fun ℓ _ => by simpa using (Finset.card_le_univ (s := views ℓ)))
    _ = L * F := by simp

theorem agreementOracleTotalWork_le
    {F L : Nat} (views : ViewFamily F L) :
    agreementOracleTotalWork views ≤ F + L * F := by
  unfold agreementOracleTotalWork
  exact Nat.add_le_add_left (agreementOracleViewWork_le views) F

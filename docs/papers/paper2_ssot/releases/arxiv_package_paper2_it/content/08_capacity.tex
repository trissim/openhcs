...
The key mathematical insight is that $n$-fold block composition turns the confusability graph into its $n$-fold \emph{strong power}:
\[
G_n \;=\; G^{\boxtimes n}.
\]
...
\paragraph{Why the strong product arises.}
The strong product structure may appear surprising at first. One might expect that if block 1 is confusable and block 2 is not, the pair should still be confusable (an ``OR-product'' over blocks). The key is that block confusability requires a \emph{single location tuple} $\ell = (\ell_1, \ell_2)$ that works \emph{simultaneously} for both blocks:
\begin{itemize}

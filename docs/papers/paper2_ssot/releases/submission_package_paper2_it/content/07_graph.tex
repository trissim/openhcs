...
The graph can be handled either explicitly or implicitly. If there are $d$ facts over an alphabet of size $q$, then the latent state space has cardinality $q^d$, so any explicit confusability graph materialization already starts from an exponentially large vertex set.\leanmeta{\LH{MFT113}} A naive explicit construction ranges over at most $q^{2d}$ ordered state pairs.\leanmeta{\LHrng{MFT}{116}{117}} For that reason the natural algorithmic interface is implicit: the formalization packages adjacency as a Boolean confusability oracle on state pairs, and proves that this oracle is equivalent to the confusability relation itself.\leanmeta{\LHrng{MFT}{114}{115}} The theory below is therefore compatible with implicit graph access even when explicit graph materialization is impractical.
...
At the same time, the exact current coordinate-subset model is \emph{not} graph-universal on the full tuple space. The formalization shows that equality at a view is equivalent to containment of that view inside the coordinate-agreement set of the two tuples, so confusability depends only on which coordinates agree, not on the actual symbols carried there.\leanmeta{\LHrng{MFT}{118}{119}, \LH{MFT122}} Equivalently, coordinatewise alphabet permutations preserve the confusability relation and induce graph automorphisms; in fact, for any two latent tuples there exists such an automorphism carrying one to the other.\leanmeta{\LHrng{MFT}{120}{124}} Thus the induced graphs in the exact tuple model form a structured subclass of finite graphs rather than an arbitrary graph family. In particular, non-vertex-transitive graphs such as a three-vertex path cannot arise as full confusability graphs in this exact model.

...
\begin{tikzpicture}[scale=0.8]
  ...
  % Region labels
  \node[blue, anchor=west] at (2.55,2.22) {\small $D = 0$ frontier};
  \node[gray] at (2.2,0.8) {\footnotesize achievable};

  % Points on the D=0 frontier
  \filldraw[orange] (0,2.8) circle (2.5pt);
  \node[orange, anchor=west] at (0.22,2.92) {\footnotesize Collision-free attr-only ($A_\pi{=}1$)};

  \filldraw[green!60!black] (5,0.6) circle (2.5pt);
  \node[green!60!black, anchor=north east] at (4.82,0.76) {\footnotesize Nominal ($D{=}0$)};
  \node[green!60!black, anchor=west] at (5.08,0.6) {\footnotesize $W=1$};

  % Probe-only strategy: below frontier, accepts D>0
  \filldraw[red] (0,1.8) circle (2.5pt);
  \node[red, anchor=west] at (0.22,1.72) {\footnotesize Colliding attr-only ($D{>}0$)};

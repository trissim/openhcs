...
\begin{theorem}[Exact semantic identity rate-distortion law on a collision block]\label{thm:positive-distortion-floor}
...
Under the hypotheses of Theorem~\ref{thm:positive-distortion-floor}, zero distortion is achievable if and only if
\[
L \ge \log_2 a.
\]
\end{corollary}
...
\begin{proof}
By Theorem~\ref{thm:positive-distortion-floor}, zero distortion is achievable exactly when $D^\star(L)=0$, equivalently when $2^L \ge a$, which is the same as $L \ge \log_2 a$.
\end{proof}
...
\begin{proof}
Set $L=0$ in Theorem~\ref{thm:positive-distortion-floor}. Since $2^0=1$, the distortion lower bound becomes $1-1/a$. Choosing a worst collision block gives the second statement.
\end{proof}

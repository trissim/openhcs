...
In the finite explicit setting, the same condition can be read directly from the representation channel: one-symbol zero-error feasibility is equivalent to injectivity. Interpreting $\pi$ as a learned embedding, this is the exact threshold for zero-error class recovery from the representation alone. \leanmeta{\LH{GPH27}}
...
\end{remark} \leanmeta{\LHrng{GPH}{44}{45}, \LH{GPH48}, \LH{GPH50}}
...
\subsection{Why the General Zero-Error Theory Simplifies Here}
...
\leanmetapending{\LH{GPH52}}
\begin{proposition}[Transitivity of representation confusability]
...
\begin{proof}
If $\pi(c_1)=\pi(c_2)$ and $\pi(c_2)=\pi(c_3)$, then $\pi(c_1)=\pi(c_3)$ by equality. Thus any two distinct classes linked through one fiber are directly confusable as well.
\end{proof}

\leanmetapending{\LHrng{GPH}{52}{53}}
\begin{proposition}[Cluster-graph structure]
...
\begin{proof}
Each fiber is a clique because all of its classes share the same representation value. No edge connects distinct fibers by construction. Hence each connected component is exactly one fiber, and the graph is a disjoint union of cliques.
\end{proof}

\leanmetapending{\LHrng{GPH}{52}{53}}
\begin{corollary}[Capacity collapse for representation-induced graphs]
...
\begin{proof}
In a disjoint union of cliques, proper coloring, clique size, and independence counting reduce to choosing or naming one element per component. In this setting, the general graph inequalities collapse to the same fiber-counting law.
\end{proof}

...
\begin{remark}[Why state the barrier explicitly]
The information barrier is immediate from the definition of representation side information: equal profiles induce identical admissible transcripts, so no attribute-only procedure can separate them. We state it explicitly because the later converse and secondary results use it as a reusable formal premise.
\end{remark}

\begin{remark}[Information-theoretic nature]
The barrier is \emph{informational}, not computational. Unlimited time or memory does not help once the representation is identical.
\end{remark}

\begin{remark}[Role in the paper]
Theorem~\ref{thm:information-barrier} is the base invariance statement. The main technical contribution is the downstream coding structure built on top of it: the ambiguity-based converse (Theorem~\ref{thm:converse}) and the adaptive side-information theorems of Section~\ref{sec:lwd}. The later query-side and robustness results reuse the same invariant in secondary ways.
\end{remark}
...
The relation $\sim$ partitions $\mathcal{V}$ into representation fibers, so an attribute-only observer operates on the quotient $\mathcal{V}/{\sim}$ rather than on raw entities. Any strategy that distinguishes values within one $\sim$-class must import new information beyond the declared representation.
\end{remark}
\leanmeta{\LHrng{ACS}{8}{9}, \LH{COH1}, \LH{COMPL1}, \LH{FORC2}}

The remaining statements in this section are axis-specialized restatements of the same barrier and are used only by later secondary results. Read through the paper's second arc, an axis map $\alpha$ is simply a candidate helper view: if all admissible observables factor through $\alpha$, then $\alpha$ is sufficient for every in-scope semantic property; if a task varies inside one $\alpha$-fiber, then that helper view is not sufficient and must be refined. These statements are therefore sufficiency-language restatements of the same residual-ambiguity theme, not a separate theorem family.

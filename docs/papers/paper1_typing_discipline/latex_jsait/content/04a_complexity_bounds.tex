...
The theorem formalizes a fact about open-world systems: present collision-freedom does not by itself yield a stable future guarantee. Persistent zero-error claims therefore need an explicit growth model rather than a one-time snapshot argument.
\end{remark}
...
\begin{proof}
Assume there is a computable certifier for $P(c):=\mathrm{HasBarrier}(\mathrm{eval}(c))$. The property is non-trivial (some generators produce collisions and some do not), so membership in the corresponding semantic class cannot be computable from code. The barrier-free statement follows by negation.
\end{proof}
...
\leanmetapending{\LHrng{ROB}{1}{2}, \LHrng{UND}{1}{2}}

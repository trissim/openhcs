...
\paragraph{The neurosymbolic linking theorem.}
We now make this precise. Let $\pi_{\mathrm{sem}}$ be a neural encoder that maps entities to semantic representations (shapes). Let $\sigma$ be a symbolic handle that maps entities to nominal identifiers. The neurosymbolic link $(\pi_{\mathrm{sem}}, \sigma)$ achieves zero-error identity recovery if and only if the handle is injective (\LH{NSL1}). This is tight: semantic representation alone cannot achieve zero-error identity when collisions exist (\LH{NSL2}), and no decoder can recover collapsed identities without identity-resolving side information (\LH{NSL3}).

The exact characterization is the helper-view theorem specialized to identity recovery: the pair $(\pi_{\mathrm{sem}}, \sigma)$ recovers identity if and only if the handle distinguishes all entities within each semantic fiber (\LH{NSL4}). An injective handle suffices (\LH{NSL5}). This architectural pattern (semantic encoder + injective nominal handle) is the canonical helper-view for open-world semantic systems (\LH{NSL6}).

In compression-theoretic terms: the neural encoder compresses semantic structure, possibly at the cost of identity collisions. The symbolic handle pays the identity bit budget. Together they achieve lossless identity compression. Neither alone suffices when the encoder is non-injective. The neurosymbolic architecture is not a design preference but an information-theoretic necessity for zero-error open-world systems.

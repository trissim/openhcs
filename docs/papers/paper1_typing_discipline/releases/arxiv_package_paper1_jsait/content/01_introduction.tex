...
\]
(Theorem~\ref{thm:semantic-identity-rate-distortion}). This makes the paper's rate-distortion contribution explicit in the deterministic corner: below the identity threshold, distortion rises according to an exact curve dictated by collision geometry. In the finite multi-fiber setting, the corresponding recoverable source mass decomposes exactly into fiberwise optima and is attained by optimizing independently within each fiber (\LH{FRD1}, \LH{FRD2}).

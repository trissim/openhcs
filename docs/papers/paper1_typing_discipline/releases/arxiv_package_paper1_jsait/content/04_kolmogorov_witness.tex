...
Let $\mathcal{C} = \{1, \ldots, k\}$ be the class space. Zero-error identification with no auxiliary side information is achievable if and only if every class has a distinct representation profile. When $\pi$ is not class-injective, no zero-error identification scheme exists without additional side information.
\end{theorem}
\leanmeta{\LH{ACS8}}
...
An \emph{auxiliary description} is a value $M(v)$ associated with each $v \in \mathcal{V}$ and transmitted in addition to the representation value $\pi(v)$. In the fixed-length setting, the auxiliary description alphabet has size $2^L$.
\end{definition}
\leanmeta{\LH{ACS7}}

\begin{theorem}[Fixed-length sufficiency]\label{thm:tag-restored-sufficiency}
An auxiliary description of length $L \geq \lceil \log_2 k \rceil$ bits suffices for zero-error identification, regardless of whether $\pi$ is class-injective.
\end{theorem}
\leanmeta{\LH{ACS7}}

\begin{proof}
A class-injective auxiliary description assigns a unique identifier to each class. Reading that description together with the representation determines $C(v)$ exactly.
\end{proof}

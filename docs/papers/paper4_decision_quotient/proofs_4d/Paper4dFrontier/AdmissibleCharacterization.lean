...
theorem no_correct_tractability_classifier_on_fullBinaryPairwiseDomain
  ...
  have hOrbit : ClosureEquivalent (dominantPairSlice 0) translatedDominantPairOrbitSlice :=
    dominantPairClosureOrbitGap_exists.1
  have hBase : HasUniqueDominantPair (dominantPairSlice 0) :=
    dominantPairClosureOrbitGap_exists.2.1
  have hNotTrans : ¬ HasUniqueDominantPair translatedDominantPairOrbitSlice :=
    dominantPairClosureOrbitGap_exists.2.2
  exact no_correctOnDomain_classifier_of_orbit_gap
    fullBinaryPairwiseDomain_closure_closed hT hCorrect trivial hOrbit hBase hNotTrans

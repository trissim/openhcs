...
\leanmetapending{\LHrng{FR}{197}{199}}
...
This answers the natural objection that Section~\ref{sec:frontier-program} excludes only classifiers that advertise closure invariance as an explicit axiom. Exact correctness already forces the same orbit agreement. The closure laws therefore supply the representation-level congruence needed for the orbit-gap program: once two slices are related by these theorem-forced presentation equivalences, any exact tractability classifier must identify them. Theorem~\ref{thm:closure-sound-package-no-go} therefore applies to any exact classifier on a closure-closed domain once the target notion is tractability itself: if the classifier separates same-orbit witnesses, it is simply wrong on at least one of them.
...
Standard exact TCS semantics are theorem instances of this transfer principle. This includes decision, counting, search, and exact approximation specifications.
...
\begin{proof}[Proof sketch]
This is the approximation-specialized form of Corollary~\ref{cor:relational-output-transfer}. An exact approximation specification is already a state-indexed admissible-output relation.
\end{proof}
...
Closure-law invariance (clause 2) is not an arbitrary stylistic restriction. Theorem~\ref{thm:correct-classifier-forces-invariance} shows that exact correctness already forces closure-orbit agreement, and Theorem~\ref{thm:closure-sound-package-no-go} shows that this theorem-forced congruence is the only load-bearing hypothesis in the no-go theorem. The remaining clauses are the minimal guardrails against the failure modes identified in Proposition~\ref{prop:oracle-loophole}; the specific guardrail formulation of clauses 1, 3, and 4 is one natural choice but is not load-bearing.

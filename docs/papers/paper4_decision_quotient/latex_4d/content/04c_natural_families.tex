...
Proposition~\ref{prop:distinct-profile-compression} is the strongest positive-side formalization the current artifact supports on the bounded-actions axis. It gives a genuine structural classifier on a tractable subcase by compression to bounded actions, but it does not package that classifier as admissible in Definition~\ref{def:admissible-normalization}, because the bounded-pattern clause there is stronger than the bounded-actions infrastructure currently formalized in Lean. The proposition still shows that nontrivial positive classification work is available through closure-law-respecting reduction inside the present framework.

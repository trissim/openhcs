...
This transfer theorem is exact rather than asymptotic. It does not assert that every external computational class falls automatically inside the optimizer-quotient framework. It states that whenever the semantic target is a deterministic payload map on the state space, exact feature sufficiency and relevance are already the same problem as exact relevance certification for the induced decision problem.
...
In particular, the transfer theorem already covers total search semantics stated as nonempty feasible-witness sets. Deterministic payloads are the singleton special case.

Partial search semantics with possible failure are likewise covered by Proposition~\ref{prop:payload-transfer} once the payload is encoded as a deterministic map $S \to \mathrm{Option}(W)$, with \texttt{none} representing failure and \texttt{some}$(w)$ representing a selected witness. Approximation semantics fit the same framework whenever the exact target to be preserved is either a deterministic approximate output or a nonempty set of admissible $\varepsilon$-solutions. What is transferred in these cases is the chosen exact approximation semantics, not an abstract approximation guarantee stated independently of a payload.
...
This removes the remaining empty-fiber caveat for search-style semantics. Set-valued payloads with failure states can be transferred without first choosing a deterministic witness selector.
...
Deterministic decision and counting outputs, total and partial search semantics, and exact approximation specifications are all special cases of this transfer principle once written as admissible-output relations.

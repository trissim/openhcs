...
\begin{tabularx}{\linewidth}{@{}>{\RaggedRight\arraybackslash}p{0.18\linewidth}>{\RaggedRight\arraybackslash}p{0.20\linewidth}>{\RaggedRight\arraybackslash}X>{\RaggedRight\arraybackslash}p{0.24\linewidth}@{}}
\toprule
...
\end{tabularx}
\par\smallskip
\textbf{Setup map.} The four central objects differ mainly in quantifier shape: minimum sufficiency collapses in the static regime, while anchor sufficiency keeps its existential fiber choice.
\endgroup
...
\leanmeta{\LH{QT7}, \LH{AB2}.}

\begin{remark}[Decision Noise]\label{rem:decision-noise}
The quotient viewpoint gives a precise meaning to decision noise. At the state level, two states differ only by decision noise exactly when they determine the same class in $Q_{\mathcal{D}}$. At the coordinate level, a coordinate is pure decision noise exactly when varying it alone never changes the decision, equivalently when it is irrelevant. In the finite probabilistic reading used later, this is also equivalent to conditional independence of the decision from that coordinate given the remaining coordinates.
\end{remark}

\leanmeta{\LHrng{DN}{1}{2}, \LH{DN5}.}

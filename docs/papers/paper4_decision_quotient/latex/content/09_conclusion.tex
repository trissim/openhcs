...
The Lean 4 development certifies the main reduction constructions, size-bounded hardness packages, exact finite deciders, counted finite-search procedures, tractability statements, explicit-state step-counting wrappers, and the scoped witness-over-verifier schema used to support the stochastic $\textsf{NP}^{\textsf{PP}}$ upper bounds. This gives a mechanically checked core for the reduction and finite-decision layers of the argument. Standard PP/PSPACE and oracle-class membership arguments remain stated in the paper rather than packaged as end-to-end Turing-machine witness proofs.

\subsection*{Outlook}

Two immediate directions remain. First, the reduction infrastructure can be integrated more tightly with general-purpose formalized complexity libraries. Second, the artifact boundary can be made even cleaner by continuing to package the existing finite-decision results into more uniform summary interfaces without changing the paper's complexity claims. On the complexity side, the remaining gap is also conceptually interesting: the stochastic anchor and minimum variants now admit paper-level $\textsf{NP}^{\textsf{PP}}$ upper bounds, but whether they collapse further to PP remains unclear because the added existential layer does not obviously disappear under the present encoding.

...
There are two closely related readings of this definition. The formal reading used in the paper and artifact is the state-based predicate above. Its intended interpretation is policy-level: hidden information may be irrelevant at time zero and still become relevant later because it changes future contingent choices. The artifact packages the certified finite core through explicit state-based verifiers and search procedures over the induced optimizer structure. The point of the sequential section is therefore not that everything reduces to a static one-step check, but that the certified state-based predicate is being used as a surrogate for temporal relevance in models where delayed informational effects matter.

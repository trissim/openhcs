...
\item \textbf{Regime Hierarchy.} We track the same regime-induced optimizer question across static, stochastic, and sequential regimes. The complexity escalates from coNP and $\Sigma_2^P$ in the static regime, to a split stochastic row in which stochastic sufficiency is polynomial-time under explicit-state encoding while a companion decisiveness family is polynomial-time under explicit-state encoding and, under succinct encoding, is PP-hard with a mechanized scoped complement upper bound of `coNP`-over-PP style and its anchor/minimum variants lie on an existential counting boundary, to PSPACE for the sequential sufficiency, anchor, and minimum queries. The stochastic anchor and minimum variants are PP-hard and admit paper-level $\textsf{NP}^{\textsf{PP}}$ upper bounds, so the paper-level landscape remains boundary-complete even though the stochastic existential cells are not yet collapsed to a single standard class.
...
\item \textbf{Lean-certified reduction and finite-decision core.} The main reductions, size-bounded hardness packages, exact finite boolean deciders, counted finite-search procedures, and explicit-state step-counting wrappers for the regime-typed query predicates are mechanized in Lean 4. For the stochastic row, the artifact now also mechanizes the explicit-state preservation checker, the scoped complement witness schema for decisiveness, the existential-majority reduction core, and an internal completeness-style packaging for the stochastic anchor family \leanmeta{\LHrng{DC}{91}{93}, \LHrng{OU}{1}{2}, \LHrng{OU}{8}{10}, \LHrng{EH}{1}{2}, \LHrng{EH}{4}{5}.} Standard PP/PSPACE and oracle-class membership arguments still remain part of the paper text rather than end-to-end Turing-machine witness proofs in the current repository.
\end{enumerate}

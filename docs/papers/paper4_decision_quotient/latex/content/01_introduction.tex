...
\item \textbf{Regime Hierarchy.} We track the same exact-certification question across static, stochastic, and sequential regimes. The complexity escalates from coNP and $\Sigma_2^P$ in the static regime, to PP in the stochastic sufficiency regime, to PSPACE for the sequential sufficiency, anchor, and minimum queries. The stochastic anchor and minimum variants sit between these endpoints: they are PP-hard and admit paper-level $\textsf{NP}^{\textsf{PP}}$ upper bounds.
...
The artifact builds with \texttt{lake build}. It certifies the reduction-correctness lemmas, size-bounded hardness packages, exact finite deciders, counted-search witnesses, tractability statements, explicit-state step-counting wrappers, and the scoped witness-over-verifier schema cited for the stochastic $\textsf{NP}^{\textsf{PP}}$ upper bounds. Standard PP/PSPACE and oracle-class membership directions remain paper proofs rather than full Turing-machine formalizations.

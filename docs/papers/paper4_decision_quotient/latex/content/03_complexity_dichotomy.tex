...
\midrule
Succinct & $k^* = \Omega(n)$ & exact certification inherits an ETH-conditioned $2^{\Omega(n)}$ lower bound & Theorem~\ref{thm:dichotomy}(2) \label{tab:dichotomy-boundary}\\
\bottomrule
\end{tabularx}
\par\smallskip
\textbf{Boundary summary.} The tractable-hard contrast is controlled jointly by encoding and by the size of the true sufficient support.
\endgroup

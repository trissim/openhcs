...
def verify_release_readiness():
    """Run the complete preflight before changing repository state."""
    try:
        subprocess.run([sys.executable, "scripts/verify_release_ready.py"], check=True)
    except subprocess.CalledProcessError as e:
        print(f"Release readiness verification failed: {e}")
        sys.exit(1)

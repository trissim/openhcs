...
"""
    ...
    def _write_lean_handle_macros_auto(self, paper_id: str) -> None:
        ...
        lines = [
            ...
            r"\renewcommand{\LH}[1]{\leavevmode\nobreak\hyperlink{lh:#1}{\mbox{\ttfamily\nolinkurl{#1}}}}",
            r"\providecommand{\LHrng}[3]{}",
            r"\renewcommand{\LHrng}[3]{\leavevmode\nobreak\hyperlink{lh:#1#2}{\mbox{\ttfamily\nolinkurl{#1#2-#3}}}}",
            r"\newif\ifleanmetaseen",
            r"\leanmetaseenfalse",
            r"\makeatletter",
            r"\providecommand{\leanmetapendinghandles}{}",
    ...
    def _write_lean_handle_ids_auto(
        ...
        lines.extend(
            [
                r"\end{list}",
                r"\else",
                r"\begin{longtable}{@{\hspace{2pt}}p{0.09\linewidth}p{0.37\linewidth}p{0.09\linewidth}p{0.37\linewidth}@{\hspace{2pt}}}",
                r"\toprule",
    ...
    def _extract_claim_label_to_lean_handles(
        ...
        for tex_file in self._iter_manual_content_tex(paper_id):
            ...
            for match in theorem_block_pattern.finditer(text):
                ...
                leading = self._extract_immediate_leading_leanmeta_block(
                    text, match.start()
                )
                if leading:
                    handles.update(extract_handles(leading))
    ...
    def _write_claim_mapping_auto(self, paper_id: str) -> None:
        ...
        id_to_handle = self._read_lean_handle_id_map(paper_id)
        handle_to_id = {handle: code for code, handle in id_to_handle.items()}
        label_to_title = self._extract_label_to_theorem_info(paper_id)
        ...
        for label in claim_labels:
            ...
            if local_handles:
                ...
                status = "Unmapped"
                unmapped_count += 1
            title = label_to_title.get(label, label)
            if handles:
                support_items: List[str] = []
                for handle in handles:
                    code = handle_to_id.get(handle)
    ...
    def build_latex(self, paper_id: str, verbose: bool = False):
        ...
        for i, (cmd, step_name) in enumerate(build_steps):
            if verbose:
            ...
            # After first pdflatex, regenerate claim_mapping with theorem numbers
            if i == 0:
                self._write_claim_mapping_auto(paper_id)
    ...
    def _extract_label_to_theorem_info(self, paper_id: str) -> Dict[str, str]:
        """Extract mapping from labels to formatted title with type and number."""
        content_dir = self._get_content_dir(paper_id)
        if not content_dir.exists():
            return {}

        label_to_info: Dict[str, Tuple[str, str]] = {}
        theorem_pattern = re.compile(
            r"\\begin\{(theorem|corollary|lemma|proposition|definition)\}"
            r"(?:\[([^\]]+)\])?"
            r"(.*?)"
            r"\\label\{((?:thm|cor|lem|prop):[^}]+)\}",
            re.DOTALL,
        )
        type_map = {
            "theorem": "Theorem",
        ...
        for tex_file in sorted(content_dir.glob("*.tex")):
            ...
            for match in theorem_pattern.finditer(text):
                ...
                label = match.group(4)
                if title and label:
                    display_type = type_map.get(env_type, "Theorem")
                    label_to_info[label] = (display_type, title)

        label_to_number = self._extract_theorem_numbers_from_aux(paper_id)

        final_mapping: Dict[str, str] = {}
        for label, (env_type, title) in label_to_info.items():
            title_escaped = title.replace("_", r"\_")
            number = label_to_number.get(label)
            if number:
                final_mapping[label] = f"{env_type} {number}: {title_escaped}"
            else:
                final_mapping[label] = title_escaped

        return final_mapping

    def _extract_theorem_numbers_from_aux(self, paper_id: str) -> Dict[str, str]:
        """Extract theorem numbers from .aux file after PDF compilation."""
        meta = self._get_paper_meta(paper_id)
        latex_dir = self._get_paper_dir(paper_id) / meta.latex_dir
        aux_file_name = meta.latex_file.replace(".tex", ".aux")
        aux_file = latex_dir / aux_file_name

        if not aux_file.exists():
            return {}

        label_to_number: Dict[str, str] = {}
        newlabel_pattern = re.compile(
            r"\\newlabel\{((?:thm|cor|lem|prop):[^}]+)\}\{"
            + r"\{(\d+|[IVXivx]+)\.(\d+)\}"
        )
        ...
        try:
            ...
            print(f"[warn] Could not read aux file {aux_file}: {e}")
    ...
    def _extract_immediate_leading_leanmeta_block(self, text: str, start: int) -> str:
        ...
        for raw_line in reversed(before_lines):
            stripped = raw_line.strip()
            if not stripped or stripped.startswith("%"):
                if leading_rev:
                    break
    ...
    def _copy_latex_sources(self, paper_id: str, package_dir: Path) -> None:
        ...
        # For arxiv packages: if latex file is not main.tex, create a main.tex wrapper
        # so arXiv can compile the paper (arXiv expects main.tex)
        meta = self._get_paper_meta(paper_id)
        if meta.latex_file != "main.tex" and not (package_dir / "main.tex").exists():
            main_wrapper = (
                "% Auto-generated by build_papers.py. Main entry point for arXiv.\n"
                f"\\input{{{meta.latex_file}}}\n"
            )
            (package_dir / "main.tex").write_text(main_wrapper, encoding="utf-8")
            print(f"[arxiv]   Created main.tex wrapper for {meta.latex_file}")

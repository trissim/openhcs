...
Simple version that doesn't use pybtex formatting to avoid errors.
"""
            ...
            if response.status_code == 200:
                ...
                if "status" in data and data["status"] == "ok":
                    ...
                    if (
                        ...
                        info = f"{title[:60]}..." if len(title) > 60 else title
                        if author_str:
...
def main():
    if len(sys.argv) < 3:
        print(
            "Usage: python validate_citations_simple.py <bib_file.bib> <tex_directory>"
        )
        print("Example: python validate_citations_simple.py references.bib content/")
        sys.exit(1)
    ...
    # Load BibTeX database (simple regex parser)
            ...
            if doi:
                ...
                if is_valid:
                    ...
                    if (
                        crossref_info
                        and title
                        and crossref_info not in title
                        and title not in crossref_info
                    ):

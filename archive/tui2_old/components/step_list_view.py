...
from .list_item_base import InteractiveListItem
...
class StepListView:
    ...
    def _get_step_display_text_for_item(self, step_core_data_dict: Dict[str, Any], is_selected: bool) -> str:
        ...
        # STATUS_ICONS would need to be accessible, e.g. from constants or passed in
        status_symbol = "?" # Placeholder
        name = step_core_data_dict.get('name', 'Unknown Step')
        func_id = step_core_data_dict.get('function_identifier', '[No Func]') # From CoreStepData
        output_info = step_core_data_dict.get('output_target', '[N/A]') # Example, adapt to CoreStepData
        
        return f"{status_symbol} {name} | {func_id} → {output_info}"

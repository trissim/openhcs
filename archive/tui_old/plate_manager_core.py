...
class PlateManagerPane:
    ...
    async def _initialize_ui(self):
        ...
        try:
            ...
            from openhcs.tui.utils.safe_formatting import error_building_component
            self.plate_items_container_widget = HSplit([error_building_component("plate list", e)])
    ...
    def _get_plate_display_text(self, plate_data: Dict[str, Any], is_selected: bool) -> str:
        ...
        # Format the display text to match the desired layout using safe formatting
        # The ^/v symbols for reordering are handled by InteractiveListItem
        from openhcs.tui.utils.safe_formatting import safe_format
        return safe_format("{status_symbol} {name} | {path_str}",
                          status_symbol=status_symbol, name=name, path_str=path_str)

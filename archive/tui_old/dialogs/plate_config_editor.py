    ...
    def _create_config_widgets(self, config_obj: Any, parent_path: str = "") -> List[Any]:
        ...
        for dc_field in dataclasses.fields(config_obj):
            ...
            if field_type == int:
                ...
                from openhcs.tui.utils.safe_formatting import SafeLabel, safe_format
                input_widget = SafeLabel(safe_format("Unsupported: {field_type}", field_type=field_type))
    ...
    def _handle_save(self) -> None:
        ...
        try:
            ...
            async def notify_apply_error():
                ...
                safe_message = safe_format("Error saving plate config: {error}", error=e)
                await self.state.notify("error_message", {"message": safe_message})

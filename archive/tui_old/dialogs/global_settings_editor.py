...
class GlobalSettingsEditorDialog:
    ...
    def _build_dialog_rows_for_model(self, model_instance: Any, model_type: Type, prefix: str = "") -> List[Any]:
        ...
        for field_name, field_obj in field_definitions.items():
            ...
            if hasattr(actual_type, 'model_fields') or dataclasses.is_dataclass(actual_type):
                # Escape field label text to prevent formatting errors
                safe_field_label = field_label_text.replace('{', '{{').replace('}', '}}')
                rows.append(VSplit([Label(f"{safe_field_label} Settings:", style="class:dialog.section-title")], height=1, padding=Dimension(left=len(prefix.split('.'))-1 if prefix else 0)))
                ...
                if current_value is None and not is_optional: # Should not happen if config is well-defined
                     ...
                     rows.append(Label(f"Error: {safe_field_label} is None", style="class:error-text"))
                     continue
                    ...
                    rows.append(Label(f"{safe_field_label}: (Not set)", style="class:italic"))
                continue
                ...
                from openhcs.tui.utils.safe_formatting import unsupported_type_label
                widget = unsupported_type_label(current_value, actual_type)
    ...
    async def _save_settings(self):
        ...
        try:
            ...
            from openhcs.tui.utils.safe_formatting import safe_text
            self.error_label.text = f"Error: {safe_text(e)}"
            get_app().invalidate()

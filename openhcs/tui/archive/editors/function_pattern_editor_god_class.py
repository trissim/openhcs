...
class FunctionPatternEditor:
    ...
    def _create_header(self):
        ...
        add_func_button = Button("Add Func",
            handler=lambda: get_task_manager().fire_and_forget(self._add_function(), "add_function"),
            width=10
        )
        load_func_button = Button("Load .func",
            handler=lambda: get_task_manager().fire_and_forget(self._load_func_pattern_from_file_handler(), "load_func"),
            width=12
        )
        save_as_func_button = Button("Save .func As",
            handler=lambda: get_task_manager().fire_and_forget(self._save_func_pattern_as_file_handler(), "save_func"),
            width=15
        )
        edit_in_vim_button = Button("Edit in Vim",
            handler=lambda: get_task_manager().fire_and_forget(self._edit_in_vim(), "edit_vim"),
            width=12
        )
    ...
    def _refresh_key_selector(self):
        ...
        def on_key_change(key):
            from openhcs.tui.utils.unified_task_manager import get_task_manager
            get_task_manager().fire_and_forget(self._switch_key(key), f"switch_key_{key}")

        key_dropdown.handler = on_key_change

        # Create key management buttons
        add_key_button = Button("Add Key", # More descriptive
            handler=lambda: get_task_manager().fire_and_forget(self._add_key(), "add_key"),
            width=10
        )
        remove_key_button = Button("Remove Key", # More descriptive
            handler=lambda: get_task_manager().fire_and_forget(self._remove_key(), "remove_key"),
            width=12
        )
        ...
        if self.is_dict:
            ...
            convert_to_dict_button = Button("Convert to Dict Pattern",
                handler=lambda: get_task_manager().fire_and_forget(self._convert_list_to_dict_pattern(), "convert_to_dict"),
                width=20
            )
    ...
    def _build_function_list(self, functions=None):
        ...
        add_function_button = Button("Add Function",
            handler=lambda: get_task_manager().fire_and_forget(self._add_function(), "add_function_2"),
            width=15
        )
    ...
    def _create_function_item(self, index, func, kwargs):
        ...
        # Function controls
        move_up = Button("↑",
            handler=lambda: get_task_manager().fire_and_forget(self._move_function_up(index), f"move_up_{index}"),
            width=3
        )
        move_down = Button("↓",
            handler=lambda: get_task_manager().fire_and_forget(self._move_function_down(index), f"move_down_{index}"),
            width=3
        )
        delete_button = Button("Delete",
            handler=lambda: get_task_manager().fire_and_forget(self._delete_function(index), f"delete_{index}"),
            width=8
        )
        ...
        item_param_editor = ParameterEditor(
            ...
            on_parameter_change=lambda p_name, p_val_str, idx=index: get_task_manager().fire_and_forget(self._handle_parameter_change(p_name, p_val_str, idx), f"param_change_{idx}"),
            on_reset_parameter=lambda p_name, idx=index: get_task_manager().fire_and_forget(self._handle_reset_parameter(p_name, idx), f"reset_param_{idx}"),
            on_reset_all_parameters=lambda idx=index: get_task_manager().fire_and_forget(self._handle_reset_all_parameters(idx), f"reset_all_{idx}")
        )
    ...
    def _create_configured_dropdown(self, dropdown_options: List[Tuple], current_func: Callable, index: int) -> GroupedDropdown:
        ...
        dropdown = GroupedDropdown(
            options_by_group=dropdown_options,
        ...
        def on_selection_change(func):
            ...
            # Skip header entries
            if func != 'header':
                get_task_manager().fire_and_forget(self._update_function(index, func), f"update_function_{index}")
    ...
    def _show_error(self, message: str):
        ...
        get_task_manager().fire_and_forget(
            show_error_dialog(title="Error", message=message, app_state=self.state), "show_error"
        )
    ...
    def _create_parameter_editor(self, func, kwargs, func_index):
        ...
        reset_all_button = Button("Reset All",
            handler=lambda: get_task_manager().fire_and_forget(self._reset_all_parameters(func_index), f"reset_all_params_{func_index}"),
            width=12
        )
    ...
    def _create_parameter_field(self, name, default, current_value, required, is_special, func_index):
        ...
        reset_button = Button("Reset",
            handler=lambda: get_task_manager().fire_and_forget(self._reset_parameter(name, default, func_index), f"reset_param_{name}_{func_index}"),
            width=8
        )
    ...
    async def _load_func_pattern_from_file_handler(self):
        ...
        try:
            ...
            loaded_pattern = await loop.run_in_executor(None, _sync_load_pattern, file_path)
    ...
    async def _save_func_pattern_as_file_handler(self):
        ...
        try:
            # Use async-safe file saving
            def _sync_save_pattern(pattern_data, path):
                with open(path, "wb") as f:
                    pickle.dump(pattern_data, f)

            loop = asyncio.get_running_loop()
            await loop.run_in_executor(None, _sync_save_pattern, self.current_pattern, file_path)

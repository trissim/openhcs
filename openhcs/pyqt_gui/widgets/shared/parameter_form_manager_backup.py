...
class ParameterFormManager(QWidget):
    ...
    def __init__(self, parameters: Dict[str, Any], parameter_types: Dict[str, type],
        ...
        debug_config = DebugConfig(enabled=False)  # Disabled by default in production
    ...
    def _create_nested_dataclass_field(self, param_name: str, param_type: type, current_value: Any) -> QWidget:
        ...
        group_box = GroupBoxWithHelp(
            title=ParameterNameFormatter.to_group_title(param_name),
        ...
        for nested_name, nested_info in nested_param_info.items():
            if self.is_global_config_editing:
                ...
                if nested_dataclass_for_form:
                    ...
                    if ParameterTypeUtils.has_resolve_field_value(nested_dataclass_for_form) and nested_current_value is not None:
        ...
        if not self._should_use_concrete_nested_values(nested_dataclass_for_form):
            ...
            nested_field_id = FieldIdGenerator.nested_static_field_id(param_name)
            ...
            if hasattr(_current_pipeline_config, 'value') and _current_pipeline_config.value:
                ...
                if nested_dataclass_instance:
                    ...
                    lazy_class = LazyDataclassFactory.create_lazy_dataclass(
                        ...
                        lazy_class_name=FieldIdGenerator.create_lazy_class_name(param_type.__name__, CONSTANTS.GLOBAL_CONTEXT_LAZY_PREFIX)
                    )
                    ...
                    lazy_class = LazyDataclassFactory.create_lazy_dataclass(
                        ...
                        lazy_class_name=FieldIdGenerator.create_lazy_class_name(param_type.__name__, CONSTANTS.STATIC_LAZY_PREFIX)
                    )
        ...
        try:
            ...
            for name, obj in vars(config).items():
                if (dataclasses.is_dataclass(obj) and
                    ParameterTypeUtils.has_dataclass_fields(obj)):
    ...
    def _create_optional_dataclass_field(self, param_name: str, dataclass_type: type, current_value: Any) -> QWidget:
        ...
        checkbox = QCheckBox(ParameterNameFormatter.to_checkbox_label(param_name))
    ...
    def _create_regular_parameter_field(self, param_name: str, param_type: type, current_value: Any) -> QWidget:
        ...
        param_description = ParameterNameFormatter.to_parameter_description(param_name, param_info.description if param_info else None)

        label_with_help = LabelWithHelp(
            text=ParameterNameFormatter.to_field_label(param_name),
    ...
    def _emit_parameter_change(self, param_name: str, value: Any):
        ...
        self.debugger.log_form_manager_operation("nested_manager_lookup", {
            "param_name": param_name,
            "parent_nested_name": parent_nested_name,
            "available_managers": list(getattr(self, 'nested_managers', {}).keys())
        })

        if parent_nested_name and hasattr(self, CONSTANTS.NESTED_MANAGERS_ATTR):
            self.debugger.log_nested_update(parent_nested_name, param_name, value)
    ...
    def _reset_parameter(self, param_name: str):
        ...
        parent_nested_name = self._find_parent_nested_manager(param_name)
        self.debugger.log_form_manager_operation("reset_debug", {
            "param_name": param_name,
            "parent_nested_name": parent_nested_name
        })
        if parent_nested_name and hasattr(self, CONSTANTS.NESTED_MANAGERS_ATTR):
            self.debugger.log_form_manager_operation("reset_parent_nested", {
                "parent_nested_name": parent_nested_name,
                "param_name": param_name
            })
            nested_manager = self.nested_managers[parent_nested_name]
    ...
    def _get_dataclass_type_for_param(self, param_name: str) -> Optional[type]:
        ...
        return ParameterTypeUtils.get_dataclass_type_for_param(param_name, self.textual_form_manager.parameter_types)

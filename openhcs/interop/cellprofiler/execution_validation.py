...
def validate_cppipe_execution(
    ...
    *,
    ...
    observation = RuntimeArtifactExecutionObservation.from_contexts(
        execution.compiled_contexts, output_root
    )
    failures = (
...
def _output_specs(prepared: PreparedGeneratedPipeline) -> tuple[ArtifactSpec, ...]:
    return tuple(
        (
            spec
...
def _runtime_exports(
    ...
    artifact_kinds: frozenset[ArtifactType],
    *,
    validate_table_exports: bool = True,
    validate_image_exports: bool = True,
) -> RuntimeExportExpectation:
    return RuntimeExportExpectation.from_flags(
        table_exports=validate_table_exports and infrastructure.exports_tables,
        image_exports=validate_image_exports and infrastructure.exports_images,
        table_artifact_kinds=frozenset(
            (kind for kind in artifact_kinds if kind.exports_as_table())
        ),
        image_export_specs=(
...
def _execution_failures(execution: DirectPipelineExecution) -> tuple[str, ...]:

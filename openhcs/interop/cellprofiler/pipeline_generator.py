...
'''
    ...
    def _load_registry(self) -> dict[str, AbsorbedModuleMetadata]:
    ...
    def _module_metadata(self, module_name: str) -> AbsorbedModuleMetadata:
    ...
    def generate_from_registry(
        ...
        runtime_module_contracts_by_module = (
            self._runtime_module_contracts_by_module_num(
                executable_modules,
                contracts_by_module,
        ...
        # Add direct backend imports for available modules.
        if executable_modules:
            imports += "# Absorbed CellProfiler functions\n"
            imports += self._backend_function_import_block(
    ...
    def _runtime_module_contracts_by_module_num(
        self,
        modules: List[ModuleBlock],
        artifact_contracts: dict[int, ModuleArtifactContracts],
        *,
        ...
        return {
            module.module_num: self._runtime_module_contract(
    ...
    def _backend_function_import_block(self, function_names: Iterable[str]) -> str:
        ...
        if not unique_function_names:
            return ""
        lines = ["from openhcs.processing.backends.cellprofiler import ("]
        lines.extend(f"    {function_name}," for function_name in unique_function_names)
        lines.append(")\n")
        return "\n".join(lines)

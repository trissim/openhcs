...
class ArtifactSpecKey:
    ...
    def from_spec(cls, spec: ArtifactSpec) -> "ArtifactSpecKey":
        return cls(artifact_type=spec.artifact_type, name=spec.name)
...
def cellprofiler_module_role(module_name: str) -> CellProfilerModuleRoleSpec:
    ...
    canonical_module_name = (
        str(module_type.module_name) if module_type is not None else normalized_name
    )
    is_infrastructure = (
    ...
    return CellProfilerModuleRoleSpec(module_name=canonical_module_name, role=role)
...
def cellprofiler_infrastructure_import_note(module_name: str) -> str:
    ...
    if module_type is not None and module_type.infrastructure_import_note is not None:
...
def cellprofiler_infrastructure_retained_artifacts(
    ...
    *,
    ...
    return module_type.infrastructure_retained_artifacts(
        module, contracts_by_module_num=contracts_by_module_num
    )

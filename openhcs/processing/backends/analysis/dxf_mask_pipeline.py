...
cp = optional_import_or_none("cupy")
...
tf = optional_import_or_none("tensorflow")
...
if HAS_TORCH:  # Keep nn.Module definition conditional on PyTorch availability
    ...
    class _RegistrationCNN_torch(
        ModulePlaceholder
    ):  # Inherit from placeholder or actual nn.Module
        def __init__(self):
    ...
    def _rasterize_polygons_slice_torch(
        ...
        for poly_tensor in polygons_gpu:  # poly_tensor shape [N_points, 2] (x, y)
            ...
            cond1 = (poly_y[j_indices] <= test_y) & (
                test_y < poly_y[k_indices]
            )  # Upward edge
            cond2 = (poly_y[k_indices] <= test_y) & (
                test_y < poly_y[j_indices]
            )  # Downward edge
            ...
            current_poly_mask_bb = torch.zeros(
                (bb_H, bb_W), dtype=torch.bool, device=device
            )
            for r_idx in range(bb_H):
                for c_idx in range(bb_W):
                    ...
                    for i in range(num_poly_pts):
                        ...
                        if (
                            abs_y > min(p1y, p2y)
                            and abs_y <= max(p1y, p2y)
                            and abs_x <= max(p1x, p2x)
                        ):
                            if p1y != p2y:  # Non-horizontal edge
                                xinters = (abs_y - p1y) * (p2x - p1x) / (
                                    p2y - p1y
                                ) + p1x
                                if (
                                    p1x == p2x or abs_x <= xinters
                                ):  # Point is to the left of edge
                                    intersections += 1
                        p1x, p1y = p2x, p2y
                    if intersections % 2 == 1:
                        current_poly_mask_bb[r_idx, c_idx] = True

            mask_slice[min_y : max_y + 1, min_x : max_x + 1] = (
                mask_slice[min_y : max_y + 1, min_x : max_x + 1] | current_poly_mask_bb
            )
        return mask_slice

    def _apply_displacement_field_torch(
        data_slice: "torch.Tensor", displacement_field: "torch.Tensor"
    ) -> "torch.Tensor":
        ...
        identity_grid = torch.stack(
            (grid_x, grid_y), dim=-1
        )  # Shape [H, W, 2] (x,y) for grid_sample
        ...
        displaced_grid = identity_grid + displacement_field.permute(
            1, 2, 0
        )  # [H, W, 2]
        displaced_grid = torch.clamp(
            displaced_grid, -1, 1
        ...
        warped_slice = F.grid_sample(
            ...
            mode="bilinear",
            padding_mode="zeros",
            align_corners=False,  # Zeros for outside mask
        )
        return warped_slice.squeeze(0)  # [C,H,W] or [H,W]

    def _smooth_field_z_torch(
        displacement_field_stack: "torch.Tensor", sigma_z: float
    ) -> "torch.Tensor":
        if sigma_z <= 0:
        ...
        coords_z = torch.arange(
            kernel_size_z, dtype=torch.float32, device=displacement_field_stack.device
        )
        coords_z -= (kernel_size_z - 1) / 2
        kernel_1d_z = torch.exp(-(coords_z**2) / (2 * sigma_z**2))
        kernel_1d_z /= kernel_1d_z.sum()

        kernel_1d_z_reshaped = kernel_1d_z.view(1, 1, kernel_size_z).repeat(
            C * H * W, 1, 1
        )  # For grouped conv

        # Reshape for 1D convolution: (N, C_in, L_in) -> (C*H*W, 1, Z)
        field_permuted = displacement_field_stack.permute(
            1, 2, 3, 0
        ).contiguous()  # [C,H,W,Z]
        field_reshaped = field_permuted.view(-1, 1, Z)  # [C*H*W, 1, Z]

        padding_z = kernel_size_z // 2
        smoothed_reshaped = F.conv1d(
            field_reshaped, kernel_1d_z_reshaped, padding=padding_z, groups=C * H * W
        )
...
def dxf_mask_pipeline(
    ...
    **kwargs,
    ...
    polygons_gpu = [
        torch.tensor(p, dtype=torch.float32, device=device) for p in dxf_polygons
    ]

    initial_rasterized_masks_float = torch.zeros(
        (Z, H, W), device=device, dtype=torch.float32
    )
    ...
    for z_idx in range(Z):
        ...
        if img_max > img_min:
            image_slice_norm = (image_slice_gray - img_min) / (img_max - img_min + 1e-6)
        else:
            image_slice_norm = torch.zeros_like(image_slice_gray)

        raster_slice = _rasterize_polygons_slice_torch(
            polygons_gpu, H, W, device
        ).float()
        initial_rasterized_masks_float[z_idx] = raster_slice

        cnn_input = torch.stack([image_slice_norm, raster_slice], dim=0).unsqueeze(
            0
        )  # [1, 2, H, W]
        with torch.no_grad():
            displacement_field_slice = registration_cnn(cnn_input).squeeze(
                0
            )  # [2, H, W]
        displacement_field_slices.append(displacement_field_slice)

    displacement_field_stack = torch.stack(
        displacement_field_slices, dim=0
    )  # [Z, 2, H, W]

    if smoothing_sigma_z > 0:
        displacement_field_stack = _smooth_field_z_torch(
            displacement_field_stack, smoothing_sigma_z
        )

    aligned_mask_slices_list = []
    for z_idx in range(Z):
        aligned_slice = _apply_displacement_field_torch(
            initial_rasterized_masks_float[z_idx], displacement_field_stack[z_idx]
    ...
    aligned_mask_stack_bool = torch.stack(
        aligned_mask_slices_list, dim=0
    )  # [Z, H, W] bool
    ...
    if apply_mask:
        ...
        if masking_mode in {DxfMaskingMode.ZERO_OUT, DxfMaskingMode.MULTIPLY}:
            ...
            return torch.where(
                mask_to_apply.bool(), masked_img_float, nans
            )  # Nan where mask is False
    else:

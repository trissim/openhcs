...
def _moving_average_1d_torch(data: torch.Tensor, window_size: int) -> torch.Tensor:
    ...
    weights = (
        torch.ones(window_size, device=data.device, dtype=data.dtype) / window_size
    )
...
def straighten_object_3d(
    ...
    **kwargs,
    ...
    if not isinstance(image_volume, torch.Tensor):
        raise TypeError(
            f"Input image_volume must be a PyTorch Tensor. Got {type(image_volume)}"
        )
    ...
    if img_vol_proc.ndim == 4:  # (1, Z, H, W)
        ...
        raise ValueError(
            f"image_volume must be 3D (Z,H,W) or 4D (1,Z,H,W). Got {image_volume.ndim}D"
        )
    ...
    if max_components_val != 1:
        ...
        raise NotImplementedError(
            "max_components > 1 is not implemented due to GPU CC complexity constraints."
        )
    ...
    object_coords_idx = torch.nonzero(
    ...
    if object_coords_idx.shape[0] == 0:
        ...
        empty_vol = torch.empty(
            (0, patch_dim, patch_dim), device=device, dtype=original_dtype
        )
        empty_grid = torch.empty(
            (0, patch_dim, patch_dim, 3), device=device, dtype=torch.float32
        )
    ...
    if centered_coords.shape[0] < 2 or torch.allclose(
        centered_coords, torch.zeros_like(centered_coords)
    ):
        logger.warning(
            "Not enough distinct object coordinates for PCA. Using a simplified centerline."
        )
        ...
        centerline_points_smooth[:, 0] = torch.linspace(
            min_z, max_z, num_fallback_points, device=device
        )
        ...
        try:
            ...
            U_pca, S_pca, V_pca_transpose = torch.linalg.svd(
                centered_coords, full_matrices=False
            )
            principal_axis = V_pca_transpose[0, :]  # First right singular vector
        except Exception as e:
            logger.warning(
                f"PCA (SVD) failed: {e}. Using simplified z-axis centerline."
            )
            min_z, max_z = object_coords[:, 0].min(), object_coords[:, 0].max()
            num_fallback_points = max(
                2, int((max_z - min_z) / sampling_spacing_val) + 1
            )
            centerline_points_smooth = torch.zeros(
                (num_fallback_points, 3), device=device
            )
            centerline_points_smooth[:, 0] = torch.linspace(
                min_z, max_z, num_fallback_points, device=device
            )
            ...
            segment_lengths_est = torch.norm(
                sorted_coords_on_axis[1:] - sorted_coords_on_axis[:-1], dim=1
            )
            curve_length_est = torch.sum(segment_lengths_est)

            spline_smoothness_val = float(
                spline_smoothness
                if spline_smoothness is not None
                else 0.01 * curve_length_est.item()
            )
            ...
            if curve_length_est > 1e-3:  # Avoid division by zero
                window_size = int(
                    max(
                        ...
                        * (num_sorted_pts / curve_length_est.item()),
                    )
                )
            else:
                window_size = 3
            window_size = min(
                window_size, num_sorted_pts // 2 if num_sorted_pts > 4 else 1
            )  # Cap window size
            if window_size < 1:
                window_size = 1
            if window_size % 2 == 0:
                window_size += 1  # must be odd for centered

            if (
                num_sorted_pts > window_size and window_size > 1
            ):  # only smooth if enough points and window > 1
                centerline_points_smooth_z = _moving_average_1d_torch(
                    sorted_coords_on_axis[:, 0].unsqueeze(0), window_size
                ).squeeze(0)
                centerline_points_smooth_y = _moving_average_1d_torch(
                    sorted_coords_on_axis[:, 1].unsqueeze(0), window_size
                ).squeeze(0)
                centerline_points_smooth_x = _moving_average_1d_torch(
                    sorted_coords_on_axis[:, 2].unsqueeze(0), window_size
                ).squeeze(0)
                centerline_points_smooth = torch.stack(
                    [
    ...
    segment_lengths = torch.norm(
        centerline_points_smooth[1:] - centerline_points_smooth[:-1], p=2, dim=1
    )
    if segment_lengths.numel() == 0:  # Single point object after smoothing/PCA
        ...
        single_slice = img_vol_proc[
            int(mean_coord[0]),
            max(0, int(mean_coord[1]) - patch_radius_val) : min(
                H_orig, int(mean_coord[1]) + patch_radius_val + 1
            ),
            max(0, int(mean_coord[2]) - patch_radius_val) : min(
                W_orig, int(mean_coord[2]) + patch_radius_val + 1
            ),
        ]
        # Pad if necessary to patch_dim x patch_dim
        padded_slice = torch.zeros(
            (patch_dim, patch_dim), device=device, dtype=original_dtype
        )
        h_s, w_s = single_slice.shape
        y_start, x_start = (patch_dim - h_s) // 2, (patch_dim - w_s) // 2
        padded_slice[y_start : y_start + h_s, x_start : x_start + w_s] = single_slice[
            :patch_dim, :patch_dim
        ].to(original_dtype)

        final_volume = padded_slice.unsqueeze(0)  # (1, patch_dim, patch_dim)
        final_grid = torch.zeros(
            (1, patch_dim, patch_dim, 3), device=device, dtype=torch.float32
        )  # Dummy grid
        return (final_volume, final_grid) if return_grid_val else final_volume

    cum_lengths = torch.cat(
        (torch.tensor([0.0], device=device), torch.cumsum(segment_lengths, dim=0))
    )
    ...
    if total_curve_length < 1e-3:  # Effectively a point
        ...
        num_samples_L = int(
            torch.ceil(total_curve_length / sampling_spacing_val).item()
        )
        if num_samples_L < 2:
            num_samples_L = 2  # Need at least 2 points for tangents

    target_cum_lengths = torch.linspace(
        0, total_curve_length, num_samples_L, device=device
    )

    resampled_centerline = torch.empty(
        (num_samples_L, 3), device=device, dtype=torch.float32
    )
    ...
    indices = torch.clamp(
        indices, 0, cum_lengths.shape[0] - 2
    ...
    sampling_grid_slices = torch.empty(
        (L, patch_dim, patch_dim, 3), device=device, dtype=torch.float32
    )

    # Create local patch coordinates (relative to plane center)
    u_coords = torch.linspace(
        -patch_radius_val, patch_radius_val, patch_dim, device=device
    )
    v_coords = torch.linspace(
        -patch_radius_val, patch_radius_val, patch_dim, device=device
    )
    grid_v_local, grid_u_local = torch.meshgrid(
    ...
    for i in range(L):
        ...
        if L == 1:
            ...
            tangent_vec = (
                resampled_centerline[i + 1] - resampled_centerline[i - 1]
            ) / 2.0

        if torch.norm(tangent_vec) < 1e-6:  # Degenerate tangent
            tangent = torch.tensor(
                [1.0, 0.0, 0.0], device=device
            )  # Default to Z-axis like
        else:
            tangent = F.normalize(
                tangent_vec, p=2, dim=0
            )  # This is new Z' (z_new_axis)
        ...
        if torch.norm(vec_u_prime_unnorm) < 1e-6:  # Arbitrary vec was parallel
            ...
            if (
                torch.abs(torch.dot(tangent, arbitrary_vec)) > 0.99
            ):  # if tangent is Z axis
                arbitrary_vec = torch.tensor([0.0, 1.0, 0.0], device=device)
            vec_u_prime_unnorm = torch.cross(tangent, arbitrary_vec)

        vec_u_prime = F.normalize(vec_u_prime_unnorm, p=2, dim=0)  # New X' (x_new_axis)
        vec_v_prime = F.normalize(
            torch.cross(tangent, vec_u_prime), p=2, dim=0
        )  # New Y' (y_new_axis)
        ...
        plane_points_world_z = (
            current_point[0]
            + grid_u_local * vec_u_prime[0]
            + grid_v_local * vec_v_prime[0]
        )
        plane_points_world_y = (
            current_point[1]
            + grid_u_local * vec_u_prime[1]
            + grid_v_local * vec_v_prime[1]
        )
        plane_points_world_x = (
            current_point[2]
            + grid_u_local * vec_u_prime[2]
            + grid_v_local * vec_v_prime[2]
        )

        # Normalize coordinates for grid_sample (expects x, y, z order in [-1, 1])
        # Original volume dimensions: Z_orig, H_orig, W_orig
        norm_coords_x = (
            2.0 * (plane_points_world_x / (W_orig - 1)) - 1.0
            if W_orig > 1
            else torch.zeros_like(plane_points_world_x)
        )
        norm_coords_y = (
            2.0 * (plane_points_world_y / (H_orig - 1)) - 1.0
            if H_orig > 1
            else torch.zeros_like(plane_points_world_y)
        )
        norm_coords_z = (
            2.0 * (plane_points_world_z / (Z_orig - 1)) - 1.0
            if Z_orig > 1
            else torch.zeros_like(plane_points_world_z)
        )

        sampling_grid_slices[i] = torch.stack(
            (norm_coords_x, norm_coords_y, norm_coords_z), dim=-1
        )
    ...
    img_vol_for_sampling = img_vol_proc.unsqueeze(0).unsqueeze(
        0
    )  # (1, 1, Z_orig, H_orig, W_orig)

    # Reshape grid for grid_sample: (N, D_out, H_out, W_out, 3)
    # Here, D_out = L, H_out = patch_dim, W_out = patch_dim
    grid_for_sampling = final_sampling_grid.unsqueeze(
        0
    )  # (1, L, patch_dim, patch_dim, 3)
    ...
    aligned_volume = aligned_volume_slices.squeeze(0).squeeze(
        0
    )  # (L, patch_dim, patch_dim)

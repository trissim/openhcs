...
class Encoder3D(nnModule):
    def __init__(self, in_channels=1, features=(32, 64), embedding_dim=128):
        ...
        self.features_channels_before_pool = features[
            1
        ]  # To know feature depth for K-Means

    def forward(
        self, x: torch.Tensor, return_features_before_pool: bool = False
        ...
        if return_features_before_pool:
            return (
                ...
                features_conv2,
            )  # Return features from last conv layer
...
class Decoder3D(nnModule):
    def __init__(
        ...
        self.init_d, self.init_h, self.init_w = (
            patch_size_dhw[0] // 4,
            patch_size_dhw[1] // 4,
            patch_size_dhw[2] // 4,
        )
        ...
        self.fc = nn.Linear(
            embedding_dim, features[0] * self.init_d * self.init_h * self.init_w
        )
        self.unflatten_channels = features[0]

        # Upsample to patch_size/2
        self.upconv1 = nn.ConvTranspose3d(
            features[0], features[1], kernel_size=4, stride=2, padding=1
        )
        # Upsample to patch_size
        self.upconv2 = nn.ConvTranspose3d(
            features[1], out_channels, kernel_size=4, stride=2, padding=1
        )
...
def _extract_random_patches(
    ...
    patches = torch.empty(
        (num_patches, 1, pD, pH, pW), device=volume.device, dtype=volume.dtype
    )
    for i in range(num_patches):
        ...
        patches[i] = volume[
            0, :, d_start : d_start + pD, h_start : h_start + pH, w_start : w_start + pW
        ]
    return patches


def _affine_augment_patch(
    patch: torch.Tensor,
) -> torch.Tensor:  # patch: [1, pD, pH, pW]
    # Random flips
...
def _nt_xent_loss(
    z_i: torch.Tensor, z_j: torch.Tensor, temperature: float
) -> torch.Tensor:
    ...
    numerator = torch.exp(
        sim_matrix[pos_mask]
    )  # Shape: [2*B] (actually B pairs, repeated)
    ...
    exp_sim_no_self = torch.exp(
        sim_matrix.masked_fill(identity_mask, -float("inf"))
    )  # Mask self-similarity
    denominator = exp_sim_no_self.sum(dim=1)  # Shape: [2*B]

    # Calculate log probabilities
    log_probs = torch.log(
        numerator / denominator[pos_mask.any(dim=1)]
    )  # Select relevant denominators
...
def _kmeans_torch(
    X: torch.Tensor, K: int, n_iters: int = 20
...
def self_supervised_segmentation_3d(
    ...
    **kwargs,
    ...
    patch_size_dhw = (
        min(patch_size_dhw[0], Z_orig),
        min(patch_size_dhw[1], H_orig),
        min(patch_size_dhw[2], W_orig),
    )
    if any(p <= 0 for p in patch_size_dhw):
        raise ValueError(
            f"Patch dimensions must be positive. Got {patch_size_dhw} for volume {Z_orig,H_orig,W_orig}"
        )

    encoder = Encoder3D(in_channels=1, embedding_dim=embedding_dim).to(device)
    decoder = Decoder3D(embedding_dim=embedding_dim, patch_size_dhw=patch_size_dhw).to(
        device
    )
    ...
    for epoch in range(n_epochs):
        ...
        patches_orig_batch = _extract_random_patches(
            img_vol_norm, patch_size_dhw, batch_size
        )
        ...
        if masks_batch.any():  # Only compute if there are masked voxels
            ...
            if (
                masked_reconstruction.numel() > 0
            ):  # If any elements were actually masked and selected
                loss_rec = F.mse_loss(masked_reconstruction, masked_original)

        loss_contrastive = _nt_xent_loss(emb_mvm, emb_affine, temperature)
        loss_bound = (
            torch.relu(reconstructed_patches - max_val_norm)
            + torch.relu(min_val_norm - reconstructed_patches)
        ).mean()

        total_loss = (
            reconstruction_weight * loss_rec
        ...
        if epoch % (n_epochs // 10 if n_epochs >= 10 else 1) == 0:
            logger.info(
                f"Epoch {epoch+1}/{n_epochs}, Loss: {total_loss.item():.4f} "
    ...
    with torch.no_grad():
        ...
        features_for_kmeans = (
            dense_features_upsampled.squeeze(0)
            .permute(1, 2, 3, 0)
            .reshape(-1, encoder.features_channels_before_pool)
        )

        if features_for_kmeans.shape[0] == 0:
            logger.warning(
                "No features extracted for K-Means, returning empty segmentation."
            )
            return torch.zeros(
                (Z_orig, H_orig, W_orig), dtype=torch.long, device=device
            )

...
def _jit_trace_batch_2d(
    ...
    for step in range(1, max_path_length):
        ...
        # --- NEW: Dynamic angular search envelope ---
        if step == 1:
            angle_span = 2 * math.pi  # full sweep for the first hop
        else:
            angle_span = node_angle_max_rad  # ±45° envelope for subsequent hops
        ...
        # --- NEW: Directional-consistency scoring with sliding window ---
        if step >= consistency_window_size:
        ...
        # --- END directional-consistency patch ---
            best_candidate_indices = torch.argmax(combined_scores, dim=1)
        else:

...
models = (
    optional_import_or_none("torchvision.models")
    if optional_import_or_none("torchvision") is not None
    else None
)
...
class HomographyPredictionNet(nnModule):
    def __init__(self, feature_dim: int):
        ...
        self.fc = nn.Sequential(
            nn.Linear(
                feature_dim * 2, 512
            ),  # Assuming features are flattened and concatenated
            nn.ReLU(),
            nn.Linear(512, 256),
            nn.ReLU(),
    ...
    def forward(self, features1: torch.Tensor, features2: torch.Tensor) -> torch.Tensor:
        ...
        flat_features1 = features1.mean(
            dim=[2, 3]
        )  # Global average pooling as a simple feature vector
...
def barlow_twins_loss(
    z1: torch.Tensor, z2: torch.Tensor, lambda_coeff: float = 5e-3
) -> torch.Tensor:
    ...
    off_diag = (
        c.masked_fill(torch.eye(D, device=c.device, dtype=torch.bool), 0).pow_(2).sum()
    )
...
def photometric_loss(
    tile_warped: torch.Tensor, tile_target: torch.Tensor
) -> torch.Tensor:
    return F.l1_loss(tile_warped, tile_target)


def warp_tile_homography(
    tile: torch.Tensor, H: torch.Tensor, output_shape: Tuple[int, int]
) -> torch.Tensor:
    ...
    grid_y, grid_x = torch.meshgrid(
        torch.linspace(-1, 1, H_out, device=tile.device),
        torch.linspace(-1, 1, W_out, device=tile.device),
        indexing="ij",
    )
    grid = torch.stack(
    ...
    safe_w_coords = torch.where(
        torch.abs(w_coords) < 1e-6, torch.sign(w_coords) * 1e-6 + 1e-9, w_coords
    )

    grid_transformed_normalized = (
        grid_transformed[:, :, :2] / safe_w_coords
    )  # [1, H_out*W_out, 2]

    # Reshape for grid_sample
    sampling_grid = grid_transformed_normalized.view(
        1, H_out, W_out, 2
    )  # [B, H_out, W_out, 2] (x,y order)

    warped_tile = F.grid_sample(
        tile, sampling_grid, mode="bilinear", padding_mode="zeros", align_corners=False
    )
    return warped_tile


def get_adjacency_from_layout(
    layout_rows: int, layout_cols: int, num_tiles: int, device: torch.device
) -> List[Tuple[int, int]]:
    """
...
def self_supervised_stitcher_func(
    ...
    *,
    ...
    Returns:
        A position tensor of shape ``[1, Z, 2]``. When
        ``return_homographies`` is enabled, returns that tensor together with
        global homographies of shape ``[Z, 3, 3]``.
    """
    ...
    if Z == 0:
        ...
        if return_homographies:
            empty_homographies = torch.empty(
                (0, 3, 3), device=device, dtype=torch.float32
            )
            return empty_positions, empty_homographies
    ...
    if layout_shape_override:
        ...
        if layout_rows * layout_cols < Z:
            raise ValueError(
                f"Provided layout_shape {layout_shape_override} is too small for {Z} tiles."
            )
    else:
    ...
    print(
        f"Using tile_shape: {tile_shape}, layout_shape: {layout_shape} for {Z} tiles."
    )
    ...
    if Z < num_layout_slots:
        ...
        padding_tensor = torch.zeros(
            padding_count, Y, X, device=device, dtype=tile_stack.dtype
        )
    ...
    global_transforms: List[torch.Tensor] = [
        torch.eye(3, device=device) for _ in range(Z)
    ]
    ...
    if learn:
        ...
        optimizer = torch.optim.Adam(
            list(feature_encoder.parameters()) + list(homography_net.parameters()),
            lr=1e-4,
        )

        # Get adjacent pairs based on layout
        adjacent_tile_pairs = get_adjacency_from_layout(
            layout_rows, layout_cols, Z, device
        )

        if Z > 1 and not adjacent_tile_pairs:
            print(
                "Warning: No adjacent pairs from layout for training, using sequential pairs as fallback."
            )
            for i_rand_pair in range(Z - 1):  # Create simple chain pairs
                adjacent_tile_pairs.append(
                    tuple(sorted((i_rand_pair, i_rand_pair + 1)))
                )
        ...
        for iter_idx in range(num_train_iterations):
            ...
            num_pairs_in_batch = (
                min(len(adjacent_tile_pairs), 8) if adjacent_tile_pairs else 0
            )
            ...
            current_batch_pairs_indices = torch.randint(
                0, len(adjacent_tile_pairs), (num_pairs_in_batch,)
            )
            ...
            for i_pair in range(idx1.shape[0]):
                ...
                pairwise_H_matrices[(p_idx1, p_idx2)] = H_12[
                    i_pair
                ].detach()  # H mapping p_idx2 to p_idx1 frame
                pairwise_H_matrices[(p_idx2, p_idx1)] = H_21[
                    i_pair
                ].detach()  # H mapping p_idx1 to p_idx2 frame

            loss_bt = barlow_twins_loss(
                features1.mean(dim=[2, 3]), features2.mean(dim=[2, 3])
            )
            ...
            tiles2_warped_to_1 = torch.stack(
                [
                    warp_tile_homography(
                        tiles2_batch[i].unsqueeze(0), H_12[i].unsqueeze(0), tile_shape
                    )
                    for i in range(tiles2_batch.shape[0])
                ]
            ).squeeze(1)
            ...
            if (
                num_pairs_in_batch > 0
                and (iter_idx + 1) % max(1, (num_train_iterations // 10)) == 0
            ):
                print(
                    f"Iter {iter_idx+1}/{num_train_iterations}, Loss: {total_loss_iter / num_pairs_in_batch:.4f}"
                )
        ...
        with torch.no_grad():
            ...
            all_graph_pairs = get_adjacency_from_layout(
                layout_rows, layout_cols, Z, device
            )
            ...
            for p1_g, p2_g in all_graph_pairs:
                ...
                if (p1_g, p2_g) not in pairwise_H_matrices:
                    ...
                    H_p1_p2 = homography_net(feat1_g, feat2_g).squeeze(
                        0
                    )  # p2 -> p1 frame
                ...
                if (p2_g, p1_g) not in pairwise_H_matrices:  # And the reverse
                    feat1_g = all_tile_features_final[p1_g].unsqueeze(0)
                    feat2_g = all_tile_features_final[p2_g].unsqueeze(0)
                    H_p2_p1 = homography_net(feat2_g, feat1_g).squeeze(
                        0
                    )  # p1 -> p2 frame
        ...
        global_transforms = optimize_pose_graph(
            pairwise_H_matrices, Z, device, initial_transforms_for_opt
        )
    ...
    for i in range(Z):
        ...
        corners_transformed_homog_i = (
            H_global_i @ tile_local_corners_homog
        )  # Shape [3, 4]
        ...
        safe_w_coords_i = torch.where(
            torch.abs(w_coords_i) < 1e-6,
            torch.sign(w_coords_i) * 1e-6 + 1e-9,
            w_coords_i,
        )

        corners_global_frame_i = (
            corners_transformed_homog_i[:2, :] / safe_w_coords_i
        )  # Shape [2, 4] (x,y)
    ...
    min_global_coords = torch.min(
        all_corners_stacked, dim=1
    ).values  # Shape [2] (min_x, min_y)

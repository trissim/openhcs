...
def _create_batch_fixed_masks_gpu(
    tile_shape: tuple, all_edge_overlaps: list, margin_ratio: float = 0.1
) -> "cp.ndarray":
    """
    ...
    if margin_pixels_y > 0:
        top_gradient = cp.linspace(
            0, 1, margin_pixels_y, endpoint=False, dtype=cp.float32
        )
        bottom_gradient = cp.linspace(
            1, 0, margin_pixels_y, endpoint=False, dtype=cp.float32
        )

    if margin_pixels_x > 0:
        left_gradient = cp.linspace(
            0, 1, margin_pixels_x, endpoint=False, dtype=cp.float32
        )
        right_gradient = cp.linspace(
            1, 0, margin_pixels_x, endpoint=False, dtype=cp.float32
        )
...
def _create_batch_dynamic_masks_gpu(
    tile_shape: tuple, all_edge_overlaps: list, overlap_fraction: float = 1.0
) -> "cp.ndarray":
    """
    ...
    for i, edge_overlaps in enumerate(all_edge_overlaps):
        if "top" in edge_overlaps:
            ...
            if overlap_pixels > 0:
                y_weights[i, :overlap_pixels] = cp.linspace(
        ...
        if "left" in edge_overlaps:
            ...
            if overlap_pixels > 0:
                x_weights[i, :overlap_pixels] = cp.linspace(
                    0, 1, overlap_pixels, endpoint=False
                )
...
def _create_dynamic_blend_mask_gpu(
    ...
    if "top" in edge_overlaps:
        overlap_pixels = int(edge_overlaps["top"] * overlap_fraction)
        if overlap_pixels > 0:
            y_weight[:overlap_pixels] = cp.linspace(
                0, 1, overlap_pixels, endpoint=False
            )
    ...
    if "left" in edge_overlaps:
        overlap_pixels = int(edge_overlaps["left"] * overlap_fraction)
        if overlap_pixels > 0:
            x_weight[:overlap_pixels] = cp.linspace(
                0, 1, overlap_pixels, endpoint=False
            )
...
def assemble_stack_cupy(
    ...
    if image_tiles.shape[0] != positions.shape[0]:
        raise ValueError(
            f"Mismatch between number of image_tiles ({image_tiles.shape[0]}) and positions ({positions.shape[0]})."
        )
    ...
    first_tile_shape = (
        tile_h,
        tile_w,
    )  # Used for blend mask, assumes all tiles same H, W
    ...
    canvas_min_x = cp.floor(min_x_pos).astype(
        cp.int32
    )  # cupy needs explicit int type for astype(int)
    canvas_min_y = cp.floor(min_y_pos).astype(
        cp.int32
    )  # cupy needs explicit int type for astype(int)

    canvas_max_x = cp.ceil(max_x_pos + tile_w).astype(
        cp.int32
    )  # cupy needs explicit int type for astype(int)
    canvas_max_y = cp.ceil(max_y_pos + tile_h).astype(
        cp.int32
    )  # cupy needs explicit int type for astype(int)
    ...
    if canvas_width <= 0 or canvas_height <= 0:
        logger.warning(
            f"Calculated canvas dimensions are non-positive ({canvas_height}x{canvas_width}). Check positions and tile sizes."
        )
        return cp.array([], dtype=cp.uint16)

    composite_accum = cp.zeros(
        (int(canvas_height), int(canvas_width)), dtype=cp.float32
    )
    ...
    if blend_method is TileBlendMethod.NONE:
        blend_masks = [
            cp.ones(first_tile_shape, dtype=cp.float32) for _ in range(num_tiles)
        ]
        ...
        if blend_method is TileBlendMethod.FIXED:
            ...
            masks_batch = _create_batch_fixed_masks_gpu(
                first_tile_shape, tile_overlaps, margin_ratio=fixed_margin_ratio
            )
        else:
            # Create all dynamic masks in one batch operation
            masks_batch = _create_batch_dynamic_masks_gpu(
                first_tile_shape, tile_overlaps, overlap_fraction=overlap_blend_fraction
            )

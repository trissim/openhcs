...
Module = create_placeholder_class(
    "Module", base_class=nn.Module if nn else None, required_library="PyTorch"
)
...
class BlurPool2d(Module):
    ...
    def __init__(self, channels: int, stride: int = 2, kernel_size: int = 3):
        ...
        if kernel_size == 3:
            ...
            kernel = torch.exp(
                -((kernel_range - (kernel_size - 1) / 2) ** 2) / (2 * sigma**2)
            )
...
def vectorized_median_replacement(
...
def extract_patches_vectorized(
    image: torch.Tensor, patch_size: int, num_patches: int
) -> torch.Tensor:
    """
    ...
    y_starts = torch.randint(
        0, y - patch_size + 1, (num_patches,), device=device, dtype=torch.long
    )
    x_starts = torch.randint(
        0, x - patch_size + 1, (num_patches,), device=device, dtype=torch.long
    )
    ...
    patches = image[
        z_indices[:, None, None], y_coords, x_coords
...
def generate_masks_vectorized(
    batch_size: int, height: int, width: int, prob: float, device: torch.device
) -> torch.Tensor:
    """Generate binary masks efficiently."""
    return (
        torch.rand(batch_size, height, width, device=device, dtype=torch.float32) < prob
    )


def process_large_slice(
    slice_2d: torch.Tensor, model: nn.Module, patch_size: int
) -> torch.Tensor:
    ...
    padded = F.pad(
        slice_2d_expanded, [pad_size, pad_size, pad_size, pad_size], mode="reflect"
    )
    ...
    for i in range(0, len(patches_list), patch_batch_size):
        ...
        for j, (y_start, y_end, x_start, x_end) in enumerate(
            positions_list[i:batch_end]
        ):
            pred_patch = batch_predictions[j]
            result[y_start:y_end, x_start:x_end] += pred_patch[
...
def n2v2_denoise_torch(
    ...
    *,
    ...
    if model_path is not None:
        ...
        patches_per_epoch = max(
            64, min(512, (z_size * y_size * x_size) // (patch_size**2))
        )
        ...
        for epoch in range(max_epochs):
            ...
            for batch_start in range(0, patches_per_epoch, batch_size):
                ...
                patches = extract_patches_vectorized(
                    image, patch_size, current_batch_size
                )
                ...
                masked_loss = (loss * masks.float()).sum() / masks.float().sum().clamp(
                    min=1
                )
            ...
            if verbose and (
                epoch % max(1, max_epochs // 5) == 0 or epoch == max_epochs - 1
            ):
                avg_loss = torch.stack(epoch_losses).mean().item()
                lr = scheduler.get_last_lr()[0]
                logger.info(
                    f"Epoch {epoch+1}/{max_epochs}, Loss: {avg_loss:.6f}, LR: {lr:.2e}"
                )
        ...
        for batch_start in range(0, z_size, slice_batch_size):
            ...
            if max(y_size, x_size) <= 512:  # Process small images directly
                ...
                for i in range(batch_end - batch_start):
                    ...
                    denoised[slice_idx] = process_large_slice(
                        slice_2d, model, patch_size
                    )

"""Self-supervised three-dimensional deconvolution inspired by Papkov et al.
...
class Deconvolution3DVolumeProjection:
    ...
    @classmethod
    def from_volume(
        cls, image_volume: "torch.Tensor"
    ) -> "Deconvolution3DVolumeProjection":
...
class _Simple3DCNN_torch(nnModule):
    ...
    def __init__(
        self, in_channels=1, out_channels=1, features=(48, 96)
    ):  # Paper: 48 initial features for 3D
        super().__init__()
...
class _LearnedBlur3D_torch(nnModule):
    def __init__(self, kernel_size=3):
        ...
        if kernel_size > 0:
            ...
            self.blur_conv.weight.data = weights.reshape(
...
def _gaussian_kernel_3d_torch(
    shape: Tuple[int, int, int], sigma: Tuple[float, float, float], device
) -> torch.Tensor:
    coords_d = (
...
def self_supervised_3d_deconvolution(
    ...
    patch_size: Tuple[int, int, int] = (
        ...
        32,
        32,
    ),  # Reduced for small test images (paper: 64x64x64)
    ...
    if img_max_orig > img_min_orig:
        img_vol_norm = (img_vol_norm - img_min_orig) / (
            img_max_orig - img_min_orig
        )  # to [0,1]
        img_vol_norm = (
            img_vol_norm * (max_val - min_val) + min_val
        )  # to [min_val, max_val]
    else:  # Constant image
    ...
    if blur_mode is DeconvolutionBlurMode.LEARNED:
        ...
        if blur_mode.uses_fixed_kernel:
            fixed_blur_kernel = _gaussian_kernel_3d_torch(
                ...
                (blur_sigma_depth, blur_sigma_spatial, blur_sigma_spatial),
                device,
            )
    ...
    for epoch in range(n_epochs):
        ...
        current_patch_orig = _extract_random_patches_torch(
            img_vol_norm.squeeze(0).squeeze(0), patch_size_dhw, 1, device
        )
        ...
        if lambda_bound > 0:
            loss_bound_f_masked = (
                torch.relu(g_f_x_masked - max_val) + torch.relu(min_val - g_f_x_masked)
            ).mean()
            loss_bound_f_orig = (
                torch.relu(g_f_x_orig - max_val) + torch.relu(min_val - g_f_x_orig)
            ).mean()
            loss_bound = (loss_bound_f_masked + loss_bound_f_orig) / 2.0

        total_loss = (
            lambda_rec * loss_rec + lambda_inv * loss_inv + lambda_bound * loss_bound
        )
        ...
        if epoch % (n_epochs // 10 if n_epochs >= 10 else 1) == 0:
            logger.info(
                f"3D Deconv Epoch {epoch}/{n_epochs}, Loss: {total_loss.item():.4f} "
    ...
    if img_max_orig > img_min_orig:
        deconvolved_final = (deconvolved_norm - min_val) / (
            max_val - min_val
        )  # to [0,1]
        deconvolved_final = (

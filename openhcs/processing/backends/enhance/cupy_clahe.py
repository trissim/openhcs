...
def _clahe_2d_vectorized(
    image: "cp.ndarray", clip_limit: float, tile_grid_size: tuple, nbins: int
) -> "cp.ndarray":
    """
...
def _apply_vectorized_interpolation_2d(
    ...
    tile_centers_y = (
        cp.arange(tile_rows, dtype=cp.float32) * tile_height + tile_height // 2
    )
    tile_centers_x = (
        cp.arange(tile_cols, dtype=cp.float32) * tile_width + tile_width // 2
    )
    ...
    bin_indices = cp.clip(
        (normalized_values * (nbins - 1)).astype(cp.int32), 0, nbins - 1
    )
...
def clahe_3d(
    ...
    if memory_efficient and total_voxels > 512**3:  # ~134M voxels threshold
        return _clahe_3d_chunked(
            stack, clip_limit, adaptive_tile_grid_3d, adaptive_nbins
        )
    else:
        return _clahe_3d_vectorized(
            stack, clip_limit, adaptive_tile_grid_3d, adaptive_nbins
        )


def _clahe_3d_vectorized(
    stack: "cp.ndarray", clip_limit: float, tile_grid_size_3d: tuple, nbins: int
) -> "cp.ndarray":
    """
        ...
        for y_idx in range(tile_y):
            for x_idx in range(tile_x):
                ...
                if cdf[-1] > 0:
                    # Normalize to [0, 1] then scale to output range
                    cdf = cdf / cdf[-1]
                    tile_cdfs[z_idx, y_idx, x_idx, :] = min_val + cdf * (
                        max_val - min_val
                    )
                else:
...
def _apply_vectorized_trilinear_interpolation(
    ...
    tile_centers_y = (
        cp.arange(tile_y, dtype=cp.float32) * tile_height + tile_height // 2
    )
...
def _clahe_3d_chunked(
    ...
    for z_start in range(0, depth, chunk_size - overlap):
        ...
        # Process chunk
        chunk_result = _clahe_3d_vectorized(chunk, clip_limit, chunk_tile_grid, nbins)
...
def _fill_3d_boundaries(
    ...
    if crop_height < height:
        full_result[:crop_depth, crop_height:, :crop_width] = cropped_result[:, -1:, :]
        if crop_depth < depth:
            full_result[crop_depth:, crop_height:, :crop_width] = cropped_result[
                -1:, -1:, :
            ]

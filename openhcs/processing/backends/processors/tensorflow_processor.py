...
# Import TensorFlow with error handling and version checking
try:
...
def create_linear_weight_mask(height: int, width: int, margin_ratio: float = 0.1) -> "tf.Tensor":
    ...
    if not HAS_TENSORFLOW:
        error_msg = f"TensorFlow is required for TensorFlowImageProcessor: {TENSORFLOW_ERROR}"
...
class TensorFlowImageProcessor(ImageProcessorInterface):
    """
    TensorFlow implementation of the ImageProcessorInterface.

    This class provides GPU-accelerated image processing operations using TensorFlow.
    All methods are stateless and operate on TensorFlow tensors.
    """

...
- Path-bearing topology belongs in one SpatialGraphArtifactType output whose SpatialGraph nodes, edges, and scalar features are authoritative. A directed morphology forest can materialize through SWCOptions while SpatialGraphROIOptions projects those same paths and features to Napari; do not reduce graph semantics to a mask or rebuild them in viewer code.
...
- The simplest canonical MetaXpress neurite step omits the step subset, inherits (Hoechst channel 1, MAP2 channel 2, SMI312 channel 4), sets input_source=PIPELINE_START and variable_components=[CHANNEL], and uses nuclear index 0, cell-body index 1, and neurite index 2. This is the compact preset's scientifically preferred Hoechst/MAP2/SMI312 model. A two-plane (SMI312, Hoechst) subset with neurite index 0 and nuclear index 1 is only the legacy shared-signal model where cell bodies are intentionally detected from SMI312 too; it is not equivalent to MAP2-seeded analysis. Retain MAP2 whenever it owns the bodies. The generic compiler regression protects any explicit ordered primary-plane subset for an implicit-main-flow callable; it is not a MetaXpress channel map.
...
- Declare path topology as SpatialGraphArtifactType and return one SpatialGraph; use SWCOptions and SpatialGraphROIOptions as projections of that same value rather than independent graph and ROI computations.
...
- Before changing biological thresholds or algorithm parameters, inspect bounded raw evidence in order. Sample the exact source and routed image planes with {agent_capabilities.sample_plate_image.name} and {agent_capabilities.sample_viewer_window_image.name}; compare dtype, shape, reported intensity statistics, bounded pixels, and nonzero/saturation behavior for the same field and coordinates. Pixel values are omitted by default from viewer sampling: explicitly request `include_array_values=true`, keep the requested tile within `max_array_elements`, and scan multiple tiles or raise that bound when the scientific claim covers more than one tile. Then inspect typed label arrays and ROI payloads with {agent_capabilities.get_viewer_window_payloads.name} and {agent_capabilities.summarize_viewer_window_rois.name}, including label identities, cardinality, bounds, and member relationships rather than display colors.
...
- For a completed UI-owned execution, list actions and follow the {results_action_id!r} action's `related_state_surface_ids` to the quantitative-results surface; never guess from a title substring. Read it with {agent_capabilities.ui_get_state_surface.name}. This is the bounded retained-table authority for quantitative troubleshooting; use runtime/debug artifact references when an intermediate or unretained value is required.
...
- The debug-session surface owns phase, session/execution identity, source group, cursor, current/last frame, snapshot-store identity, and exact disabled reasons. Wait for an operation receipt with {agent_capabilities.ui_wait_for_operation_receipt.name}, then re-read the surface; do not repeatedly invoke an action while it is pending.
...
- Validate and sample payloads with {agent_capabilities.validate_viewer_window_state.name}, {agent_capabilities.sample_viewer_window_image.name}, and {agent_capabilities.summarize_viewer_window_rois.name}; use ROI summaries and nonzero payload checks as concrete transport evidence, then use exact raw-versus-result pixels or rasterized shapes for scientific QA. Layer existence and nonzero pixels prove transport, not scientific adequacy: also verify that the final layer encodes the requested objects, measurements, or relationships across the claimed field."""
...
- List recovery state with {agent_capabilities.ui_list_snapshots.name} and {agent_capabilities.ui_list_branches.name}. Use {agent_capabilities.ui_restore_snapshot.name} for an approved exact rollback, {agent_capabilities.ui_time_travel_head.name} to return to the current branch head, and {agent_capabilities.ui_switch_branch.name} only for an intentional branch change. Re-read the affected field and state surface after every recovery mutation."""

...
def migrate_cache_file(legacy_path: Path, new_path: Path, description: str) -> bool:
    ...
    if new_path.exists():
        logger.debug(
            f"XDG {description} already exists at {new_path}, skipping migration"
        )
        return False
    ...
    try:
        ...
        logger.warning(
            f"Failed to migrate {description} from {legacy_path} to {new_path}: {e}"
        )
        return False
...
def migrate_cache_directory(legacy_dir: Path, new_dir: Path, description: str) -> bool:
    ...
    if new_dir.exists() and any(new_dir.iterdir()):
        logger.debug(
            f"XDG {description} already exists and is not empty at {new_dir}, skipping migration"
        )
        return False
    ...
    try:
        ...
        logger.warning(
            f"Failed to migrate {description} from {legacy_dir} to {new_dir}: {e}"
        )
        return False
...
def migrate_all_legacy_cache_files() -> None:
    ...
    migrations = [
        (
            legacy_dir / "path_cache.json",
            get_openhcs_data_dir() / "path_cache.json",
            "path cache",
        ),
        (
            legacy_dir / "global_config.config",
            get_openhcs_config_dir() / "global_config.config",
            "global config cache",
        ),
    ]
    ...
    if migrated_files > 0:
        logger.info(
            f"Successfully migrated {migrated_files} cache files/directories to XDG locations"
        )
    else:

...
class SystemMonitorTextual(Container):
    ...
    def compose(self) -> ComposeResult:
        ...
        if PLOTEXT_AVAILABLE:
            # Use Grid layout for plots with two combined graphs in single row
            with Grid(id="plots_grid"):
                yield PlotextPlot(id="cpu_gpu_plot")
                yield PlotextPlot(id="ram_vram_plot")
        else:
...
╚══════════════════════════════════════════════════════════════╝"""
    ...
    def _update_plots(self) -> None:
        ...
        try:
            ...
            # Update CPU/GPU combined plot
            try:
                cpu_gpu_plot = self.query_one("#cpu_gpu_plot", PlotextPlot)
                cpu_gpu_plot.plt.clear_data()
                
                # Plot CPU usage
                if self.monitor.cpu_history and len(self.monitor.cpu_history) > 0:
                    cpu_gpu_plot.plt.plot(x_range, list(self.monitor.cpu_history), label="CPU", marker="braille", color="cyan")
                
                # Plot GPU usage if available
                if GPU_AVAILABLE and self.monitor.gpu_history and any(self.monitor.gpu_history):
                    cpu_gpu_plot.plt.plot(x_range, list(self.monitor.gpu_history), label="GPU", marker="braille", color="yellow")
                    title = f"CPU: {current_cpu:.1f}% | GPU: {current_gpu:.1f}%"
                else:
                    title = f"CPU: {current_cpu:.1f}% | GPU: N/A"
                    
                cpu_gpu_plot.plt.title(f"Processing Unit Usage - {title}")
                cpu_gpu_plot.plt.ylim(0, 100)
                cpu_gpu_plot.plt.xlabel("Time (seconds)")
                cpu_gpu_plot.plt.ylabel("Usage %")
                cpu_gpu_plot.plt.legend()
                cpu_gpu_plot.refresh()
            except Exception as e:
                import logging
                logging.getLogger(__name__).debug(f"CPU/GPU plot update failed: {e}")

            # Update RAM/VRAM combined plot
            try:
                ram_vram_plot = self.query_one("#ram_vram_plot", PlotextPlot)
                ram_vram_plot.plt.clear_data()
                
                # Plot RAM usage - ensure we have data
                if self.monitor.ram_history and len(self.monitor.ram_history) > 0:
                    ram_vram_plot.plt.plot(x_range, list(self.monitor.ram_history), label="RAM", marker="braille", color="green")
                
                # Plot VRAM usage if available
                if GPU_AVAILABLE and self.monitor.vram_history and any(self.monitor.vram_history):
                    ram_vram_plot.plt.plot(x_range, list(self.monitor.vram_history), label="VRAM", marker="braille", color="magenta")
                    memory_title = f"RAM: {current_ram:.1f}% | VRAM: {current_vram:.1f}%"
                else:
                    memory_title = f"RAM: {current_ram:.1f}% | VRAM: N/A"
                    
                ram_vram_plot.plt.title(f"Memory Usage - {memory_title}")
                ram_vram_plot.plt.ylim(0, 100)
                ram_vram_plot.plt.xlabel("Time (seconds)")
                ram_vram_plot.plt.ylabel("Usage %")
                ram_vram_plot.plt.legend()
                ram_vram_plot.refresh()
            except Exception as e:
                import logging
                logging.getLogger(__name__).debug(f"RAM/VRAM plot update failed: {e}")

        except Exception as e:
            import logging
            logging.getLogger(__name__).debug(f"Overall plot update failed: {e}")

...
def _pose_loss_fn(translation: jnp.ndarray, quaternion: jnp.ndarray, 
                  base_coords: jnp.ndarray, receptor_coords: jnp.ndarray,
    ...
    energy = _score_single_lj(receptor_coords, pose_coords, receptor_radii, ligand_radii)
    return energy


@jax.jit(static_argnames=['n_steps'])
def optimize_poses_batched(translations: jnp.ndarray, quaternions: jnp.ndarray, 
                           ...
                           n_steps: int = 50, 
                           lr_t: float = 0.05, 
                           lr_q: float = 0.05) -> tuple[jnp.ndarray, jnp.ndarray]:
    """
    ...
    # Differentiate w.r.t. translation and quaternion only (argnums 0,1)
    value_and_grad_fn = jax.value_and_grad(_pose_loss_fn, argnums=(0, 1))
    
    def _optimize_single(t: jnp.ndarray, q: jnp.ndarray) -> tuple[jnp.ndarray, jnp.ndarray]:
        def step_fn(i, val):
            curr_t, curr_q = val
            energy, (grad_t, grad_q) = value_and_grad_fn(
                curr_t, curr_q, ligand_base_coords, receptor_coords,
            ...
            grad_t_dir = jnp.where(grad_t_norm > 1e-6, grad_t / grad_t_norm, jnp.zeros_like(grad_t))
            grad_q_dir = jnp.where(grad_q_norm > 1e-6, grad_q / grad_q_norm, jnp.zeros_like(grad_q))
            
            # Clip step sizes: max 0.1A translation, max 0.05rad rotation per step
    ...
    batched_optimize = jax.vmap(_optimize_single, in_axes=(0, 0))

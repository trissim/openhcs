...
class CPPipeSettingCoverageCollector:
    ...
    def for_case(
        self, case: CPPipeCorpusCase, modules: Sequence[ModuleBlock]
        ...
        missing_processing_rows = tuple(
            (
                self.for_module_setting(
                    case, module, setting, CPPipeSettingCoverage.MODULE_NOT_ABSORBED
                )
                for module in partition.processing_modules
        ...
        infrastructure_rows = tuple(
            (
                self.for_module_setting(
                    case, module, setting, CPPipeSettingCoverage.INFRASTRUCTURE
                )
        ...
        try:
            ...
            return (
                ...
                *(
                    self.for_module_setting(
                        case, module, setting, CPPipeSettingCoverage.GENERATION_ERROR
                    )
    ...
    def for_bound_setting(
        self, case: CPPipeCorpusCase, coverage: ModuleSettingCoverageRecord
...
def build_cellprofiler_compatibility_report(
    *,
    ...
    corpus_observations = _corpus_module_observations(
        parser or CPPipeParser(), corpus_cases or default_cppipe_corpus()
    )
    modules = tuple(
        (
            _module_compatibility_coverage(module_name, corpus_observations.coverage)
    ...
    return CellProfilerCompatibilityReport(
        ...
        benchmark_coverage=_benchmark_coverage_summary(
            corpus_observations, modules, cppipe_modules
        ),
    )
...
def _module_compatibility_coverage(
    ...
    if importable:
        try:
            resolved_contract = resolve_processing_contract(module_name, function_name)
    ...
    return ModuleCompatibilityCoverage(
        ...
        corpus_coverage=corpus_coverage.get(
            module_name, ModuleCorpusCoverage.NOT_IN_CORPUS
        ),
    )
    ...
    for module in modules:
        ...
        family_supported_modules = tuple(
            (
                ...
                if modules_by_name[family_module_name].corpus_coverage
...
def _cellprofiler_source_module_names(
    ...
    root = (
        source_modules_root
        or Path(__file__).resolve().parents[1] / "cellprofiler_source" / "modules"
    )
    if not root.exists():
        return ()
    return tuple(
...
def _corpus_module_observations(
    parser: CPPipeParser, corpus_cases: Sequence[CPPipeCorpusCase]
    ...
    for case in corpus_cases:
        ...
        for module_name in module_names:
            coverage[module_name] = _merged_corpus_coverage(
                coverage.get(module_name), case_coverage
            )
        ...
        if case.status is CPPipeCorpusStatus.SUPPORTED:
            setting_coverage.extend(setting_coverage_collector.for_case(case, modules))
...
def _benchmark_coverage_summary(
    ...
    return CellProfilerBenchmarkCoverageSummary(
        ...
        known_invalid_cppipe_case_count=corpus_observations.known_invalid_cppipe_case_count,
...
def _merged_corpus_coverage(
    existing: ModuleCorpusCoverage | None, candidate: ModuleCorpusCoverage

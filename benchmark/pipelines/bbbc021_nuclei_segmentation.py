#!/usr/bin/env python3
"""
OpenHCS Pipeline - BBBC021 Nuclei Segmentation (CellProfiler-equivalent)

This pipeline replicates CellProfiler's IdentifyPrimaryObjects for BBBC021 dataset.
CellProfiler parameters from ExampleHuman.cppipe:
- Typical diameter: 8-80 pixels
- Threshold strategy: Global
- Thresholding method: Minimum Cross-Entropy
- Declumping: Intensity
- Fill holes: After declumping only
- Discard border objects: Yes

BBBC021: Human MCF7 cells - DAPI (nuclei), tubulin, actin
...
# CellProfiler preprocessing parameters (from ExampleHuman.cppipe)
# Gaussian sigma = diameter / 3.5 = 8 / 3.5 ≈ 2.3
GAUSSIAN_SIGMA = 2.3
...
step_1 = FunctionStep(
    func=(preprocess, {
        ...
        'median_size': 0,  # Disabled by default
    }),
...
step_2 = FunctionStep(
    func=(segment, {
        'min_diameter': 8,                          # CellProfiler: 8 pixels min
        'max_diameter': 80,                         # CellProfiler: 80 pixels max
        'threshold_method': 'minimum_cross_entropy', # CellProfiler: Min Cross-Entropy
        'threshold_correction': 1.0,
        'declump_method': 'intensity',              # CellProfiler: Intensity declumping
        'fill_holes': True,

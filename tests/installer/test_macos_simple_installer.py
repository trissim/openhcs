...
def test_macos_installer_ci_drives_and_captures_the_shipping_app() -> None:
    ...
    assert "AXUIElementCreateApplication(processIdentifier)" in probe
    assert "kAXWindowsAttribute" in probe
    assert "kAXDefaultButtonAttribute" in probe
    assert "kAXEnabledAttribute" in probe
    assert "AXUIElementPerformAction(button, kAXPressAction as CFString)" in probe
    assert "CGEvent(" not in probe

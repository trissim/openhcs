...
def test_windows_installer_ci_has_an_absolute_safety_ceiling() -> None:
    ...
    assert "$releaseVersion = $env:OPENHCS_INSTALLER_CANDIDATE_VERSION" in smoke_step
    assert "Resolve-Path $env:OPENHCS_INSTALLER_CANDIDATE_WHEELHOUSE" in smoke_step
...
def test_windows_installer_ci_drives_and_captures_the_shipping_wizard() -> None:
    ...
    assert '"OpenHCS Installer\\bootstrap.log"' in workflow
...
def test_windows_installer_ci_surfaces_runtime_evidence_on_failure() -> None:
    workflow = INTEGRATION_WORKFLOW_PATH.read_text(encoding="utf-8")
    ...
    assert 'Join-Path $installRoot "gui-data\\openhcs\\logs"' in failure_step
    assert "Get-ChildItem -LiteralPath $runtimeLogRoot -File" in failure_step

...
def test_plate_manager_expands_multi_cppipe_folder_into_orchestrators(
    ...
    def record_orchestrator(self, plate_path: str, *, plate_root=None):
        created_orchestrators.append((plate_path, Path(plate_root)))
        return None
    ...
    assert created_orchestrators == [
        (expected_first_scope, plate_root),
        (expected_second_scope, plate_root),
    ]
    ...
    assert widget.selected_plate_path == expected_second_scope
    close_widget(widget)

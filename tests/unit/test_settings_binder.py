...
def test_opening_module_class_inherits_structuring_element_binding():
    module = ModuleBlock(
        name="Opening",
        module_num=1,
        settings={"Structuring element": "Disk,5"},
    )
...
def test_gray_to_color_rescale_default_follows_cellprofiler_revision_upgrade():
    ...
    assert _bind_declared_module_settings(
        v3_module,
...
def test_define_grid_alias_settings_bind_invocation_options_on_module_declaration():
    ...
    assert (
        bound.invocation_options.cycle_scope
        is DefineGridCycleScope.ONCE
    )
...
def test_module_settings_binding_allows_derived_dead_output_setting_ignore():
    ...
    bound = _bind_module_settings(
        module,
        ignored_unmapped_settings=frozenset({"unexpected_setting"}),
    )
        ...
        (
            ModuleBlock(
                ...
                setting_records=[
                    ...
                    ModuleSetting(
                        "Retain the outlines of filtered objects for use later "
                        "in the pipeline (for example, in SaveImages)?",
                        "Yes",
                    ),
...
def test_module_only_helper_bindings_use_declared_module_path(
    module,
    expected_kwargs,
):
    bound = _bind_declared_module_settings(module)

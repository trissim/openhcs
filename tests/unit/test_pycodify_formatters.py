...
def test_dtype_conversion_serializes_through_openhcs_public_import():
    source = _source(DtypeConfig(default_dtype_conversion=DtypeConversion.UINT8))

    assert "from openhcs.core.memory import DtypeConversion" in source
    assert "arraybridge.decorators" not in source

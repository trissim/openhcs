...
def test_cellprofiler_backend_function_identity_survives_catalog_rebuild():
    ...
    assert crop is cellprofiler_backend.CellProfilerFunctionCatalog.get_function("crop")
...
def test_cellprofiler_backend_catalog_has_no_undocumented_configurable_parameters():
    ...
    for function_name in (
        cellprofiler_backend.CellProfilerFunctionCatalog.list_functions()
    ):
        func = cellprofiler_backend.CellProfilerFunctionCatalog.get_function(
            function_name
        )
        ...
        for parameter_name, parameter in inspect.signature(func).parameters.items():
            if parameter.kind in _SIGNATURE_TRANSPORT_PARAMETER_KINDS:
                continue
            if (
                parameter_name not in analyzed
                or not analyzed[parameter_name].description
            ):

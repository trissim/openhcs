...
class SemanticAnalyzer:
    ...
    def __init__(self, device: str = "cuda", config: Optional[Dict[str, Any]] = None):
        ...
        self.type_checker = TypeChecker(device=self.device)
        self.variable_resolver = VariableResolver(device=self.device)
        self.function_resolver = FunctionResolver(device=self.device)
    ...
    def analyze(
        ...
        if "complexity" in analysis_types:
            ...
            results["complexity"] = {
                key: value.item() for key, value in complexity_metrics.items()
            }
        ...
        if "dependency" in analysis_types:
            ...
            results["dependency"] = {
                key: value.cpu().numpy() if value.numel() > 0 else np.array([])
        ...
        if "type" in analysis_types:
            ...
            results["types"] = {
                "types": type_results["types"].cpu().numpy(),
                "errors": type_results["errors"].cpu().numpy()
            }

        # Variable resolution
        if "variable" in analysis_types:
            variable_results = self.variable_resolver.resolve_variables(ast_tensors)
            results["variables"] = {
                "declarations": variable_results["declarations"].cpu().numpy(),
                "references": variable_results["references"].cpu().numpy(),
                "resolution": variable_results["resolution"].cpu().numpy()
            }

        # Function resolution
        if "function" in analysis_types:
            function_results = self.function_resolver.resolve_functions(ast_tensors)
            results["functions"] = {
                "declarations": function_results["declarations"].cpu().numpy(),
                "calls": function_results["calls"].cpu().numpy(),
                "resolution": function_results["resolution"].cpu().numpy()
            }
    ...
    def cleanup(self) -> None:
        ...
        if self.device == "cuda":
            try:
                ...
                if hasattr(self.type_checker, 'cleanup'):
                    self.type_checker.cleanup()

                if hasattr(self.variable_resolver, 'cleanup'):
                    self.variable_resolver.cleanup()

                if hasattr(self.function_resolver, 'cleanup'):

...
# Import SMA plugin interface
from semantic_matrix_analyzer.plugins import SMAPlugin, PluginContext
...
class GPUAnalysisPlugin(SMAPlugin, ConfigObserver):
    ...
    def analyze_code(self, code: str, file_path: Optional[Union[str, Path]] = None,
        ...
        try:
            ...
            # Extract patterns from code if pattern analysis is requested
            patterns = []
            if analysis_types is None or "pattern" in analysis_types:
                # Initialize pattern extractor if needed
                if not hasattr(self, 'pattern_extractor'):
                    try:
                        from .pattern_extraction import PatternExtractor

                        # Check if dynamic_config_manager exists
                        if not hasattr(self, 'dynamic_config_manager'):
                            logger.warning("dynamic_config_manager not found, initializing with default config")
                        ...
                        logger.debug("Initialized pattern extractor")
                    except Exception as e:
                        logger.warning(f"Could not initialize pattern extractor: {e}")
                        import traceback
                        logger.warning(traceback.format_exc())
                        self.pattern_extractor = None

                # Extract patterns from code
                if hasattr(self, 'pattern_extractor') and self.pattern_extractor is not None:
                    logger.debug("Extracting patterns from code")
                    try:
                        patterns = self.pattern_extractor.extract_patterns(code, "code", {
                            "file_path": str(file_path) if file_path else None
                        })
                        logger.debug(f"Extracted {len(patterns)} patterns from code")

                        # Add patterns to semantic analyzer
                        for pattern in patterns:
                            self.add_pattern(pattern)
                    except Exception as e:
                        logger.warning(f"Error extracting patterns: {e}")

            # Analyze code with semantic analyzer
            results = self.semantic_analyzer.analyze(code, file_path, analysis_types)

            # Add pattern matches to results
            if patterns:
                logger.debug("Adding pattern matches to results")
                pattern_matches = []

                # Match patterns against code
                if hasattr(self, 'pattern_extractor') and self.pattern_extractor is not None:
                    try:
                        logger.debug(f"Matching {len(patterns)} patterns against code")

                        # Add patterns directly to results
                        pattern_info = []
                        for pattern in patterns:
                            pattern_info.append({
                                "id": pattern.id,
                        ...
                        results["patterns"] = pattern_info
                        logger.debug(f"Added {len(pattern_info)} patterns to results")

                        # Match patterns
                        for pattern in patterns:
                            logger.debug(f"Matching pattern: {pattern.name} ({pattern.pattern_type})")
                            matches = self.pattern_extractor.match_patterns(code, pattern.pattern_type)
                        ...
                        logger.debug(f"Added {len(pattern_matches)} pattern matches to results")
                    except Exception as e:
            ...
            results["metadata"].update({
                ...
                "num_pattern_matches": len(results.get("pattern_matches", [])) if "pattern_matches" in results else 0
            })
